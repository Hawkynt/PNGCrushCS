...
public readonly record struct ScreenBlasterFile : IImageFormatReader<ScreenBlasterFile>, IImageToRawImage<ScreenBlasterFile>, IImageFromRawImage<ScreenBlasterFile>, IImageFormatWriter<ScreenBlasterFile> {
  ...
  public static ScreenBlasterFile FromRawImage(RawImage image) {
    ArgumentNullException.ThrowIfNull(image);
    if (image.Format != PixelFormat.Indexed8)
      throw new ArgumentException("RawImage must use PixelFormat.Indexed8.", nameof(image));
    if (image.Width != 320)
      throw new ArgumentException("Screen Blaster images must be exactly 320 pixels wide.", nameof(image));
    if (image.Height != 200)
      throw new ArgumentException("Screen Blaster images must be exactly 200 pixels tall.", nameof(image));
