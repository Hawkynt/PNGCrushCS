...
public sealed class ImageFormatGenerator : IIncrementalGenerator {
  ...
  private const string _IMAGE_FROM_TYPED_RAW_IMAGE = "FileFormat.Core.IImageFromRawImage`2";
  ...
  private static ImmutableArray<FormatInfo> _DiscoverFormats(Compilation compilation, System.Threading.CancellationToken ct) {
    ...
    var imageFromTypedRawImage = compilation.GetTypeByMetadataName(_IMAGE_FROM_TYPED_RAW_IMAGE);
    ...
    foreach (var type in allSymbols) {
      ...
      var typedWritePixelFormats = new List<string>();
      ...
      foreach (var iface in type.AllInterfaces) {
        ...
        if (typeArgs.Length == 2) {
          if (imageFromTypedRawImage != null
              && SymbolEqualityComparer.Default.Equals(def, imageFromTypedRawImage)
              && SymbolEqualityComparer.Default.Equals(typeArgs[0], type))
            typedWritePixelFormats.Add(typeArgs[1].ToDisplayString(SymbolDisplayFormat.FullyQualifiedFormat));
          continue;
        }
  ...
  private static void _EmitTypedWriteCapabilities(StringBuilder sb, FormatInfo format) {
    if (format.TypedWritePixelFormats.Length == 0) {
      sb.Append("System.Array.Empty<RawImageWriteCapability>()");
      return;
    }

    sb.Append("new RawImageWriteCapability[] { ");
    for (var i = 0; i < format.TypedWritePixelFormats.Length; ++i) {
      if (i > 0)
        sb.Append(", ");

      sb.Append("_TypedWriteCapability<")
        .Append(format.FullTypeName)
        .Append(", ")
        .Append(format.TypedWritePixelFormats[i])
        .Append(">()");
    }
    sb.Append(" }");
  }
  ...
  private static void _GenerateRegistration(SourceProductionContext spc, ImmutableArray<FormatInfo> formats, string targetNamespace) {
    ...
    foreach (var format in formats) {
      ...
      var hasWriter = format.HasFormatReader && format.HasToRawImage && format.HasFromRawImage && format.HasFormatWriter;
      var method = hasWriter ? "_RegisterReaderWriter" : "_RegisterReader";
