...
/// <summary>
/// Encodes ayuv: 4:4:4:4 YUV with alpha and nothing compressed at all, four bytes a pixel and no
/// chroma subsampling to decide about.
/// </summary>
/// <remarks>
/// The mirror of <see cref="AyuvVideoDecoder"/>, and the layout is that decoder's own: four bytes a
/// pixel — V, then U, then Y, then alpha, the reverse of what the name spells and not the order v408
/// uses — repeating across a row with no padding of any kind, a row exactly <c>width</c> times four
/// bytes and no header ahead of the picture. Nothing is predicted and
...
/// </remarks>
public sealed class AyuvVideoEncoder : IVideoCodecEncoder<AyuvVideoEncoder> {
  ...
  private AyuvVideoEncoder(MediaStreamInfo stream) {
  ...
  public static AyuvVideoEncoder Create(MediaStreamInfo stream) {
    ArgumentNullException.ThrowIfNull(stream);
    if (stream.Kind != MediaStreamKind.Video || stream.Width <= 0 || stream.Height <= 0)
      throw new NotSupportedException(
        $"ayuv encoding requires a video stream with positive dimensions; stream {stream.Index} states "
  ...
  public bool TryEncode(RawImage frame, long? presentationTimestamp, out CodedPacket packet) {
    ...
    if (frame.Width != this._stream.Width || frame.Height != this._stream.Height)
      throw new InvalidDataException(
        $"ayuv geometry is fixed at {this._stream.Width}x{this._stream.Height}; received {frame.Width}x{frame.Height}.");
    ...
    for (var row = 0; row < this._stream.Height; ++row) {
      ...
      for (var x = 0; x < width; ++x) {
        ...
        line[offset] = cr[pixel];
        line[offset + 1] = cb[pixel];
        line[offset + 2] = luma[pixel];
  ...
  private static RawImage _ToPlanar(RawImage frame) => frame.Format switch {
    ...
    _ => throw new NotSupportedException(
      $"ayuv takes {PixelFormat.Yuv444P8} samples as they are, or {PixelFormat.Rgba32}, {PixelFormat.Bgra32} and "
