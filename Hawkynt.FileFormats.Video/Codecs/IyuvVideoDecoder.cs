...
/// <summary>
/// Decodes IYUV: uncompressed 4:2:0 YUV, three separate planes, and nothing else.
/// </summary>
/// <remarks>
/// <b>The layout.</b> A full-resolution luma plane, then a whole Cb plane, then a whole Cr plane,
/// each chroma plane half the width and half the height of the picture. Byte for byte this is
/// <c>I420</c>: the two codes are one layout under two names, kept as two types because a stream
/// states one of them and an encoder has to be able to write back whichever it was asked for. A row
/// of any plane is exactly its own width in bytes, there is no padding anywhere, and rows run top
/// row first.
/// <para/>
/// <b>Verified against ffmpeg's own decode, exactly.</b> Files ffmpeg 9 wrote as <c>rawvideo</c> at
/// <c>-pix_fmt yuv420p -vtag IYUV</c> were decoded here and compared sample for sample against
...
/// </remarks>
public sealed class IyuvVideoDecoder : IVideoCodecDecoder<IyuvVideoDecoder> {

  private static readonly CodecTag _Tag = CodecTag.FromCharacters("IYUV");

  private readonly Yuv420Packing _packing;

  private IyuvVideoDecoder(Yuv420Packing packing) => this._packing = packing;
  ...
  public static IyuvVideoDecoder Create(MediaStreamInfo stream)
    => new(Yuv420Packing.For(stream, Yuv420Order.PlanarCbCr, "IYUV"));
