...
/// <summary>
/// Encodes UYVY: uncompressed 4:2:2 YUV, two pixels to four bytes — Cb, Y0, Cr, Y1 — and nothing
/// else.
/// </summary>
/// <remarks>
/// The mirror of <see cref="UyvyVideoDecoder"/>, writing the layout that decoder reads back: Cb,
/// luma, Cr, luma in every four bytes, a row exactly <c>width</c> times two bytes with no padding,
...
/// uyvy422</c> encoder wrote for the same pictures — all 7,800 of them.
/// <para/>
...
public sealed class UyvyVideoEncoder : IVideoCodecEncoder<UyvyVideoEncoder> {
  ...
  private UyvyVideoEncoder(MediaStreamInfo stream, PackedYuv422Packing packing) {
  ...
  public static string CodecName => "Uncompressed packed 4:2:2 (UYVY)";

  public static CodecTag Codec => _Tag;

  public static UyvyVideoEncoder Create(MediaStreamInfo stream) {
    ArgumentNullException.ThrowIfNull(stream);

    if (stream.Kind != MediaStreamKind.Video)
      throw new NotSupportedException("UYVY can only encode a video stream.");

    return new(stream, PackedYuv422Packing.For(stream, PackedYuv422Order.CbLumaCrLuma, "UYVY"));
  }
  ...
  public bool TryEncode(RawImage frame, long? presentationTimestamp, out CodedPacket packet) {
    ...
    if (frame.Width != this._stream.Width || frame.Height != this._stream.Height)
      throw new InvalidDataException(
        $"UYVY geometry is fixed at {this._stream.Width}x{this._stream.Height}; received {frame.Width}x{frame.Height}.");
