...
/// <summary>
/// Decodes UYVY: uncompressed 4:2:2 YUV, two pixels to four bytes — Cb, Y0, Cr, Y1 — and nothing
/// else.
/// </summary>
/// <remarks>
/// <b>The layout.</b> A macropixel is two horizontally adjacent pixels and four bytes: Cb, luma, Cr,
/// luma. Each pair of pixels states its own chroma pair and shares it; a row is exactly <c>width</c>
/// times two bytes with no padding, and rows run top row first. It is <c>YUY2</c> rotated by one
/// byte, not a different sample grid; this is the ordering ffmpeg calls <c>uyvy422</c> and the one
/// this package's avui decoder already unpacks.
/// <para/>
/// <b>Verified against ffmpeg's own decode, exactly.</b> Files ffmpeg 9 wrote as <c>rawvideo</c> at
/// <c>-pix_fmt uyvy422 -vtag UYVY</c> were decoded here and compared sample for sample against
...
/// </remarks>
public sealed class UyvyVideoDecoder : IVideoCodecDecoder<UyvyVideoDecoder> {

  private static readonly CodecTag _Tag = CodecTag.FromCharacters("UYVY");

  private readonly PackedYuv422Packing _packing;

  private UyvyVideoDecoder(PackedYuv422Packing packing) => this._packing = packing;
  ...
  public static UyvyVideoDecoder Create(MediaStreamInfo stream)
    => new(PackedYuv422Packing.For(stream, PackedYuv422Order.CbLumaCrLuma, "UYVY"));
