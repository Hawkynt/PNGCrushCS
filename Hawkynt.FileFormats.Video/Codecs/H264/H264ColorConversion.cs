...
/// </summary>
/// <remarks>
/// Both steps here are display conventions rather than parts of the coding standard. H.264 codes
...
/// is not the same in H.264 as in MPEG-1. H.264's default siting (<c>chroma_sample_loc_type</c> 0,
/// inherited from MPEG-2) puts a chrominance sample level with the even luminance column and halfway
/// between the two luminance rows it covers. So bringing chrominance back up is an exact copy on even
/// columns and a halfway average on odd ones, while vertically it is the three-to-one interpolation a
/// quarter-step offset calls for. Using MPEG-1's centre siting instead shifts every colour edge half a
/// luminance sample to the left, which is small, everywhere, and looks like a decode that worked.
/// </remarks>
internal static class H264ColorConversion {

  /// <summary>
  /// Crops a decoded picture to its displayed size and converts it to packed 8-bit RGB.
  /// </summary>
  /// <param name="picture">The reconstructed planes, which are a whole number of macroblocks across.</param>
  /// <param name="left">The first displayed column, from the sequence parameter set's cropping offsets.</param>
  ...
  internal static byte[] ToRgb24(H264Picture picture, int left, int top, int width, int height) {
    var rgb = new byte[width * height * 3];

    // The chrominance samples that belong to the displayed picture, which is not the whole plane. A
    // cropped stream is coded a whole macroblock wider or taller than it is shown, and the samples
    // past the crop are real reconstructed samples that later pictures may predict from — but they
    // are not part of this picture, so the interpolation replicates the last displayed sample rather
    // than reaching into them. Reaching in is a colour edge in the last row and column of every
    // cropped picture, small enough to look like rounding.
    ...
    for (var y = 0; y < height; ++y) {
      var lumaRow = (top + y) * picture.LumaWidth + left;
      var target = y * width * 3;

      for (var x = 0; x < width; ++x) {
        var luma = picture.Luma[lumaRow + x];
        var cb = _Chroma(picture.Cb, picture.ChromaWidth, left + x, top + y, chromaLeft, chromaRight, chromaTop, chromaBottom);
        var cr = _Chroma(picture.Cr, picture.ChromaWidth, left + x, top + y, chromaLeft, chromaRight, chromaTop, chromaBottom);
