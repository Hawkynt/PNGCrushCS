...
public sealed class H263VideoEncoder : IVideoCodecEncoder<H263VideoEncoder> {
  ...
  /// <remarks>
  /// H.263 states no group length of its own -- there is no GOP header in the baseline syntax -- so
  /// this is a rate decision rather than a syntax one. Twelve bounds how far a decoder that joined
  /// mid-stream has to wait, and how far an error can propagate, to eleven pictures.
  /// </remarks>
  ...
  /// <summary>Driven on this encoder's own output, to hand back the reference a predicted picture uses.</summary>
  ...
  /// <summary>Encodes one picture: intra at the head of a group, forward-predicted otherwise.</summary>
  public bool TryEncode(RawImage frame, long? presentationTimestamp, out CodedPacket packet) {
    ...
    // samples its decoder will hold rather than on the frame that was handed in. The two differ by
    // the quantiser's loss, and an encoder that ignores that is right on its first predicted picture
    // and a little further out on every one after.
