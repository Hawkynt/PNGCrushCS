...
/// <para/>
/// Measured against ffmpeg 9.0.1 on FATE's <c>tdsc/tdsc.asf</c>: raw tiles agree to the byte, and the
/// cursor paths agree with what ffmpeg returns for hand-built streams. One thing is deliberately not
/// copied. The reference decoder declares its picture BGR24, copies raw tiles into it byte for byte,
/// and then writes red first for every pixel of a JPEG tile — so one frame carrying both comes out
/// with two channel orders in it. Everything written here is BGR. See <c>codec-notes.md</c>.
/// </remarks>
public sealed class TdscVideoDecoder : IVideoCodecDecoder<TdscVideoDecoder> {
  ...
  private const uint _Raw = 0x52415720;  // " WAR", the little-endian spelling of "RAW "
  ...
  /// <summary>What every container this codec appears in actually writes.</summary>
  private static readonly CodecTag _Tag = CodecTag.FromCharacters("TDSC");

  /// <summary>
  /// The transposition the reference decoder's own file header states. No sample carries it, but a
  /// stream that did would be this codec, so it is taken as well rather than refused.
  /// </summary>
  private static readonly CodecTag _TransposedTag = CodecTag.FromCharacters("TSDC");
  ...
  public static bool Accepts(MediaStreamInfo stream) {
    ...
    return stream.Kind == MediaStreamKind.Video
      && (stream.Codec.EqualsIgnoringCase(_Tag) || stream.Codec.EqualsIgnoringCase(_TransposedTag));
  }
  ...
  public bool TryDecode(CodedPacket packet, out RawImage frame) {
    ...
    // Anything else is a block this codec does not know. The reference decoder consumes the tag,
    // paints nothing and still hands back the canvas, so a stream carrying one keeps playing.
  ...
  private void _DecodeJpegTile(ReadOnlySpan<byte> encoded, int x, int y, int width, int height) {
    // A tile that will not decode leaves that part of the canvas as it was. The reference decoder
    // does the same unless it is told to explode: one unreadable tile is a smear, not a dead file.
    RawImage decoded;
    try {
  ...
  private void _DecodeCursorRecord(ref LittleEndianReader reader) {
    ...
    switch (action) {
      ...
      case 3:
        ...
        break;
      default:
        // An action this codec does not know leaves the cursor where it was; the reference decoder
        // asks for a sample and carries on rather than failing the frame.
        break;
    }
  }
  ...
  private void _LoadCursor(ref LittleEndianReader reader) {
    ...
    if (this._cursorHotX > this._cursorWidth || this._cursorHotY > this._cursorHeight) {
      this._cursorHotX = Math.Min(this._cursorHotX, this._cursorWidth - 1);
      this._cursorHotY = Math.Min(this._cursorHotY, this._cursorHeight - 1);
    }
