...
/// <summary>
/// Decodes YVYU: uncompressed 4:2:2 YUV, two pixels to four bytes — Y0, Cr, Y1, Cb — and nothing
/// else.
/// </summary>
/// <remarks>
/// <b>The layout.</b> A macropixel is two horizontally adjacent pixels and four bytes: luma, Cr,
/// luma, Cb. Each pair of pixels states its own chroma pair and shares it; a row is exactly
/// <c>width</c> times two bytes with no padding, and rows run top row first. It is <c>YUY2</c> with
/// the chroma pair exchanged, which is exactly the difference a wrong reading of either one produces
/// — the picture still decodes and its reds and blues are swapped.
/// <para/>
/// <b>Verified against ffmpeg's own decode, exactly.</b> Files ffmpeg 9 wrote as <c>rawvideo</c> at
/// <c>-pix_fmt yvyu422 -vtag YVYU</c> were decoded here and compared sample for sample against
...
/// </remarks>
public sealed class YvyuVideoDecoder : IVideoCodecDecoder<YvyuVideoDecoder> {

  private static readonly CodecTag _Tag = CodecTag.FromCharacters("YVYU");

  private readonly PackedYuv422Packing _packing;

  private YvyuVideoDecoder(PackedYuv422Packing packing) => this._packing = packing;
  ...
  public static YvyuVideoDecoder Create(MediaStreamInfo stream)
    => new(PackedYuv422Packing.For(stream, PackedYuv422Order.LumaCrLumaCb, "YVYU"));
