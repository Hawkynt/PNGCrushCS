...
/// edge of the screen the reference decoder clips its height and width but still reads the sprite from
/// its own first row and column, drawing the wrong part of the pointer; the source is offset here as
/// well. See <c>codec-notes.md</c> for how that was measured and what else was compared.
/// </remarks>
public sealed class VmncVideoDecoder : IVideoCodecDecoder<VmncVideoDecoder> {
  ...
  public bool TryDecode(CodedPacket packet, out RawImage frame) {
    ...
    for (var chunk = 0; chunk < chunks; ++chunk) {
      if (reader.Remaining < 12)
        throw new InvalidDataException(
          $"VMnc stream {this._streamIndex} ends inside rectangle {chunk} of {chunks}.");
    ...
    for (var tileY = 0; tileY < height; tileY += 16) {
      ...
      for (var tileX = 0; tileX < width; tileX += 16) {
        ...
        for (var index = 0; index < subrectangles; ++index) {
          ...
          // A subrectangle is four bits of offset and four of size, so it can reach 31 pixels and
          // spill past its own 16-pixel tile. The bound that matters is the rectangle being
          // painted, not the tile: this is what the reference decoder allows, and a stricter test
          // rejects streams that decode.
          if (rectX + rectWidth > width - tileX || rectY + rectHeight > height - tileY)
            throw new InvalidDataException($"VMnc stream {this._streamIndex} carries a Hextile subrectangle outside its rectangle.");
  ...
  /// <remarks>
  /// An n-bit channel is widened by repeating its own bits, which is what the reference decoder's
  /// output format means and what every other 16-bit codec in this package does — a five-bit 3 is
  /// 24, not the 25 a proportional rounding would give. A maximum that is not one less than a power
  /// of two is not a bit field at all, and only then is the proportional form used.
  /// </remarks>
  private static byte _Scale(uint value, uint maximum) {
    var bits = BitOperations.PopCount(maximum);
    if (bits >= 32 || maximum != (1u << bits) - 1u)
      return checked((byte)((value * 255u + maximum / 2u) / maximum));
    if (bits >= 8)
      return (byte)(value >> (bits - 8));

    var widened = value;
    for (var filled = bits; filled < 8; filled += bits) {
      widened = (widened << bits) | value;
      if (filled + bits >= 8)
        return (byte)(widened >> (filled + bits - 8));
    }
    return (byte)widened;
  }
