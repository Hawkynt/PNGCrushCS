...
internal static class H263BlockEncoder {
  ...
  /// <summary>
  /// Quantises a prediction residual and answers whether anything survived, which is what puts the
  /// block in the coded block pattern.
  /// </summary>
  /// <remarks>
  /// H.261 and H.263 share this quantiser exactly -- both reconstruct an odd multiple of the step
  /// size and pull an even step one towards zero -- so the forward step is the one already written
  /// for H.261 rather than a second version of the same arithmetic here.
  /// </remarks>
  internal static bool QuantiseInter(
    scoped ReadOnlySpan<int> residual, int quantiser, scoped Span<int> levels)
    => H261BlockEncoder.QuantiseInter(residual, quantiser, levels);

  /// <summary>Writes one inter block: no DC field, every coefficient through Table 16.</summary>
  /// <remarks>
  /// An inter block has no separate DC value -- the prediction already carries the average -- so the
  /// scan starts at nought rather than at one, and the block is only reached at all when the coded
  /// block pattern named it.
  /// </remarks>
  internal static void WriteInter(H263BitWriter writer, scoped ReadOnlySpan<int> levels)
    => _WriteCoefficients(writer, levels, firstScan: 0);
  ...
  internal static void WriteIntra(
    ...
    _WriteCoefficients(writer, levels, firstScan: 1);
  }

  private static void _WriteCoefficients(
    H263BitWriter writer, scoped ReadOnlySpan<int> levels, int firstScan) {
    var lastScan = 63;
    while (lastScan > firstScan && levels[H263Quantisation.ZigZag[lastScan]] == 0)
      --lastScan;

    var previous = firstScan - 1;
