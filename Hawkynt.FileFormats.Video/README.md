...
| [Electronic Arts CMV](https://wiki.multimedia.cx/index.php/Electronic_Arts_CMV) | ⚠️ | ✅ | ffmpeg | `cmv `; eight-bit palettised pictures. Intra pictures are the complete raster; inter pictures are 4x4 blocks that copy an exact motion-compensated block from the previous picture, copy from the second-last picture through an escape, or carry sixteen literal indices. `MVIh` may change geometry and resets motion history. The lossless encoder writes full and partial palette headers, chooses the smaller of raw intra and exact two-reference inter coding, and refuses true-colour or alpha-bearing input rather than quantising it. Inter coding is emitted only for complete 4x4 grids; odd geometry remains intra. CMV has no forward-reference or B-picture syntax | [MultimediaWiki EA CMV](https://wiki.multimedia.cx/index.php/Electronic_Arts_CMV) |
