...
| [MPEG-4 Part 2 video (ISO/IEC 14496-2)](https://en.wikipedia.org/wiki/MPEG-4_Part_2) | ⚠️ | ✅ | ffmpeg | Rectangular, progressive, 8-bit 4:2:0 I/P/B. Quarter-sample vectors, sprites/GMC, interlace, OBMC, data partitioning, scalability, shape coding, newpred, reduced resolution and the complexity-estimation header each refused where signalled. The encoder writes Advanced Simple I/P/B streams at quantiser 8, with an I-VOP every twelve displayed pictures and two B-VOPs between anchors: P-VOPs use zero-vector temporal prediction plus residuals, and each B macroblock chooses forward, backward or interpolated zero-vector prediction by pre-quantisation squared error. Every packet repeats its VOL so VFW-style containers do not depend on out-of-band configuration. MPEG-4 Part 2 has no lossless mode in this coding path; every coded block is transformed and quantised | [ISO/IEC 14496-2](https://www.iso.org/standard/39259.html) |
...
- Encoding is a smaller domain than decoding on purpose: 47 codecs of the 100 read can also be written. Most are lossless; thirteen are not, and each says so in its own row. Motion JPEG writes baseline JPEG, whose loss is a matter of degree, and DV's fixed-length frames, Microsoft Video 1's two-colours-to-a-block coding, Cinepak's vector quantisation, both ISO MPEG-4 Part 2 and Microsoft's MPEG-4 transform coding, the ASUS codecs' quantised DCT, Apple Video's 15-bit blocks, H.261's and Apple ProRes's quantised transforms, Hap's texture blocks and id RoQ's vector quantisation have no lossless form at all — a picture that codec can hold exactly comes back exactly, and one it cannot does not. What is still not written back is the newer lossy codecs: a stream read as H.264 cannot be written back as H.264.
