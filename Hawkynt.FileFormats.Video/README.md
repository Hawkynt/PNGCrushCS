...
| [ZeroCodec](https://wiki.multimedia.cx/index.php/ZeroCodec) | ⚠️ | ✅ | ffmpeg | `ZECO`; the independently verified layout is zlib-compressed, bottom-up UYVY 4:2:2 at 16 bits a pixel. Key packets are literal I-pictures; P-pictures use zero bytes to copy from the immediately preceding picture. The encoder writes that layout, promoting a picture to I when a changed byte becomes zero because the P predictor cannot express that transition. There are no B-pictures, future references or frame reordering. Other depths advertised by the original VFW codec are refused because no independent wire-layout description or sample was found | [FFmpeg `zerocodec.c`](https://github.com/FFmpeg/FFmpeg/blob/master/libavcodec/zerocodec.c) / [original ZeroCodec release](https://gigazine.net/gsc_news/en/20090709_zerocodec/) |
