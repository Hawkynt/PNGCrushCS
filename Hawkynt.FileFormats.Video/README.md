...
Microsoft's MPEG-4, ASUS V1 and V2, Apple Video, H.261, VC-1, Hap, Apple ProRes, id RoQ — the row says so, because there is nothing else such an encoder
...
| [VC-1 / Windows Media Video 9 (SMPTE 421M, Simple and Main profile intra pictures)](https://en.wikipedia.org/wiki/VC-1) | ⚠️ | ✅ | ffmpeg | `WMV3`; Simple and Main profile **intra pictures**, with the sequence header read from container private data, plus skipped pictures after a decoded picture by repeating its samples. Predicted and bidirectional pictures, Advanced profile (`WVC1`, `WMVA`), MULTIRES, RANGERED and LOOPFILTER each refuse by name. The encoder writes Main-profile progressive all-intra `WMV3` at uniform quantiser 3; each 8x8 block carries only its predicted DC coefficient, making a deliberately coarse block-average picture, but every packet is independently decodable and valid VC-1 | [SMPTE ST 421](https://ieeexplore.ieee.org/document/7290900) |
...
- Encoding is a smaller domain than decoding on purpose: 47 codecs of the 100 read can also be written. Most are lossless; thirteen are not, and each says so in its own row. Motion JPEG writes baseline JPEG, whose loss is a matter of degree, and DV's fixed-length frames, Microsoft Video 1's two-colours-to-a-block coding, Cinepak's vector quantisation, Microsoft's MPEG-4 transform, the ASUS codecs' quantised DCT, Apple Video's 15-bit blocks, H.261's and Apple ProRes's quantised transforms, VC-1's DC-only block averages, Hap's texture blocks and id RoQ's vector quantisation have no lossless form at all — a picture that codec can hold exactly comes back exactly, and one it cannot does not. What is still not written back is the modern lossy codecs: a stream read as H.264 cannot be written back as H.264.
