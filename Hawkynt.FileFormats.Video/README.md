...
| [LCL MSZH](https://wiki.multimedia.cx/index.php/Lossless_Codec_Libraries) | ⚠️ | ✅ | ffmpeg | `MSZH`; RGB24 only on decode. Reads compressed, uncompressed and two-section MSZH packets. The encoder writes single-section bottom-up RGB24 with DWORD-padded rows, using MSZH back-references when they shrink the frame and the reference decoder's equal-size raw-frame form otherwise | [FFmpeg `lcldec.c`](https://github.com/FFmpeg/FFmpeg/blob/master/libavcodec/lcldec.c) |
