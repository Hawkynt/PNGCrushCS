...
| [Autodesk FLIC](https://en.wikipedia.org/wiki/FLIC_(file_format)) | `Fli` | `.fli`, `.flc`, `.flx`, `.flh`, `.flt` | ✅ | ✅ | none | [MultimediaWiki FLIC](https://wiki.multimedia.cx/index.php/FLIC) |
...
| [FLIC](https://en.wikipedia.org/wiki/FLIC_(file_format)) | ⚠️ | ✅ | ffmpeg | `FLIC`; 8-bit palettised FLI/FLC, Autodesk/Ulead 15-bit RGB555 FLX, and AF44 DTA 15/16/24-bit RGB555/RGB565/BGR24 over a persistent previous-frame canvas. Standard type-7 `SS2` obeys its two-bit line-opcode classes and uses word packets; AF44 type-27 `DTA_LC` instead treats any negative line word as a skip and counts packet skips/runs in pixels. Tempra FLX differs only in making type-7 packet skips byte counts and is not distinguished from Autodesk/Ulead AF12 by a reliable header marker. The lossless encoder writes `SS2` deltas for indexed streams and `DTA_LC` deltas under AF44 for true colour, falling back to `BRUN`/`DTA_BRUN` whole pictures whenever they are smaller; unchanged pictures carry no image chunk. RGB555 input must already be exactly representable and is never silently quantised | [CompuPhase FLIC](https://www.compuphase.com/flic.htm) |
