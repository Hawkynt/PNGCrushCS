...
- Encoding is a smaller domain than decoding on purpose: 23 codecs of the 82 read can also be written, and all but one of them are lossless or format-faithful. The exception is Microsoft Video 1, whose two-colours-to-a-block coding has no lossless form at all; it says so in its own row and in its own remarks. Nothing here transcodes a picture into a lossy codec whose loss is a matter of degree, so a stream read as H.264 cannot be written back as H.264.
