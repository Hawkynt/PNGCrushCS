...
| [Matroska](https://en.wikipedia.org/wiki/Matroska) / [WebM](https://www.webmproject.org/) | `Matroska` | `.mkv`, `.mka`, `.mks`, `.mk3d`, `.webm` | ✅ | ✅ | ffmpeg | [Matroska elements](https://www.matroska.org/technical/elements.html) / [WebM container](https://www.webmproject.org/docs/container/) |
...
| [id RoQ](https://wiki.multimedia.cx/index.php/RoQ) | ⚠️ | ✅ | ffmpeg | `RoQV`; quadtree vector quantisation with motion compensation over two picture buffers, plus the older Trilobyte/11th Hour extensions: `RoQ_JPEG` intraframes, alpha-bearing ten-byte codebook cells, `RoQ_HANG` repeat pictures, legacy motion scaling and `RoQ_PACKET` wrappers. RoQ defines no B pictures or future references: `FCC` reads the immediately previous reconstructed picture, while `MOT` leaves the alternating target buffer untouched. The encoder writes the standard id/Quake III quadtree form — one packet per picture carrying `INFO`, `QUAD_CODEBOOK` and `QUAD_VQ` — sizing a codebook per picture and pricing every block between skip, motion, doubled 4x4 cells and subdivision. It is lossy because RoQ is. Mid-stream size changes and pictures whose sides are not a whole number of 16-pixel macroblocks are refused by name rather than guessed or padded | [MultimediaWiki RoQ](https://wiki.multimedia.cx/index.php/RoQ) |
