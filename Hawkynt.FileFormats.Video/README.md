...
Microsoft's MPEG-4, ASUS V1 and V2, Apple Video, H.261, Hap, Apple ProRes, id RoQ, Intel Indeo 4 — the row says so, because there is nothing else such an encoder
...
all: those were written from published descriptions. The Indeo 4 encoder is independently derived
from the format behavior already expressed by this package's decoder and shared IVI layer; ffmpeg is
used as an interoperability oracle, not copied as encoder implementation code. ProRes takes FFmpeg's quantisation matrices,
...
- Encoding is a smaller domain than decoding on purpose: 47 codecs of the 100 read can also be written. Most are lossless; thirteen are not, and each says so in its own row. Motion JPEG writes baseline JPEG, whose loss is a matter of degree, and DV's fixed-length frames, Microsoft Video 1's two-colours-to-a-block coding, Cinepak's vector quantisation, Microsoft's MPEG-4 transform, the ASUS codecs' quantised DCT, Apple Video's 15-bit blocks, H.261's and Apple ProRes's quantised transforms, Hap's texture blocks, id RoQ's vector quantisation and Intel Indeo 4's YVU9 chroma subsampling have no lossless RGB form at all — a picture that codec can hold exactly comes back exactly, and one it cannot does not. What is still not written back is the modern lossy codecs: a stream read as H.264 cannot be written back as H.264.
