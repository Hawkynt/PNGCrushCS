...
[`VideoFormatRegistry.CreateDecoder`](https://github.com/Hawkynt/PNGCrushCS/blob/main/Hawkynt.FileFormats.Video/VideoFormatRegistry.cs) builds, 102 of them; `Encode` is
what [`VideoFormatRegistry.CreateEncoder`](https://github.com/Hawkynt/PNGCrushCS/blob/main/Hawkynt.FileFormats.Video/VideoFormatRegistry.cs) builds, 71 of them. The two
...
| [H.265/HEVC (ITU-T H.265 \| ISO/IEC 23008-2)](https://en.wikipedia.org/wiki/High_Efficiency_Video_Coding) | ⚠️ | ✅ | ffmpeg | Main profile, 8-bit 4:2:0; intra and inter slices, reference management, CABAC, weighted prediction, scaling lists, deblocking, SAO, tiles and dependent slices. The writer opens each group of twelve with a Main-profile IDR picture made from 32x32 PCM coding units — stored exactly, so the only loss there is the 4:2:0 conversion RGB input goes through — and codes the rest as P and B pictures in the order I P B B P B B: 16x16 inter coding units with integer-sample motion over a 16-sample search, merge and skip where the candidate already names the motion found, explicit vectors against the advanced predictor where it does not, a choice per block between the past anchor, the future anchor and the average of the two, and a quantised transform residual. Pictures are coded out of display order and carry their own count, so a decoder reorders them. VPS/SPS/PPS travel in `HEVCDecoderConfigurationRecord` private data with length-prefixed samples. What it does not do is fractional-sample motion, more than one reference per list, more than one prediction unit per coding unit, or any rate-distortion search over coding unit sizes — each costs compression, none costs conformance. The decoder handles that uniform PCM shape as well as its general slice path; arbitrary PCM coding units elsewhere, the format range extensions, screen-content coding, multilayer/3D and separate colour planes are refused | [ITU-T H.265](https://www.itu.int/rec/T-REC-H.265) |
...
| [On2 VP5](https://en.wikipedia.org/wiki/VP5) | ✅ | — | — | `VP50` entire: intra and predicted pictures, all ten macroblock modes over the previous and the golden reference, one motion vector or four, half-sample luminance and quarter-sample chrominance prediction with the edge filter that goes with it, direct-current prediction from up to four neighbours, and interlaced field-coded macroblocks. No encoder: nothing outside On2 writes VP5 — ffmpeg has a decoder and no encoder for it — so there is nothing to measure one against | [MultimediaWiki On2 VP5](https://wiki.multimedia.cx/index.php/On2_VP5) |
...
| Hap | ⚠️ | ✅ | ffmpeg, Vidvox Hap + bcdec | `Hap1`, `Hap5`, `HapY`, `HapM`, `HapA`, `Hap7`, `HapH`. Unknown texture formats, second-stage compressors, and image combinations Hap does not define are refused. The encoder writes all seven variants: BC1, BC3, Scaled YCoCg, Scaled YCoCg + BC4 alpha, BC4 alpha-only, BC7, and BC6H; `HapH` selects unsigned or signed BC6H from the finite input samples. Frames may be emitted as one texture section or as equal block-aligned chunks, with each chunk independently stored raw or Snappy-compressed. Hap is intra-only, so every packet is a key frame with no forward/backward reference state. DXT/BC texture coding is lossy except where its quantised endpoint/index representation states the source samples exactly. ffmpeg remains the independent reader for the classic Hap variants; Vidvox `HapDecode` plus bcdec independently validate the Hap R BC7 and signed/unsigned Hap HDR BC6H output | [Vidvox Hap `HapVideoDRAFT.md`](https://github.com/Vidvox/hap/blob/master/documentation/HapVideoDRAFT.md) |
...
| [Avid Meridien Compressed](https://wiki.multimedia.cx/index.php/Motion_JPEG) | ⚠️ | ✅ | ffmpeg | `AVDJ`; Avid JFIF, which is baseline Motion JPEG with two layout rules the JPEG itself does not carry. A packet whose coded picture is under three quarters of the stated frame height is two complete JPEG field pictures back to back and is woven into one frame, padding between them skipped; anything taller is one whole picture. A coded picture taller than the container’s geometry is Avid macroblock padding above the image, so the bottom rows and left columns are what the display keeps, and a container claiming more than its packet codes is refused rather than filled in. Which output rows the first coded field belongs on comes from QuickTime’s `fiel` extension, else the Video-for-Windows Avid discriminator (1 NTSC, 2 PAL) alone or behind a `BITMAPINFOHEADER`, else the two D1 rasters whose order Avid fixes — 486-line NTSC stores its lower field first, 576-line PAL its upper; a two-field packet at any other geometry with nothing saying which way round is refused rather than guessed. **Deliberately not claimed**: the alpha-bearing Meridien variant, which no public bitstream description defines — neither a second JPEG nor a four-component JPEG is reinterpreted as alpha on read, and a picture carrying non-opaque alpha is refused on write rather than flattened. The encoder writes baseline 4:2:2 JPEG at one fixed quality, 720x486 and 720x576 as two fields per packet in temporal order with a `fiel` of BB and TT respectively, every other geometry progressive; interlace follows the geometry because a stream description carries no field order to ask. Only the progressive case claims the Matroska `V_MJPEG` identity, a two-field packet under which would read back as its first field alone. Every packet is a key frame and JPEG is lossy, so nothing round-trips exactly. Measured against ffmpeg 8.1.2: three five-frame clips written here — 720x486, 720x576 and a progressive 320x240 — decoded without a message, every frame compared against the picture that went in, mean absolute error 1.1 interlaced and 1.9 progressive, where writing the display-only `fiel` forms instead measures 134 | [MultimediaWiki Motion JPEG](https://wiki.multimedia.cx/index.php/Motion_JPEG) |
...
[ffmpeg](https://ffmpeg.org/) covers most codecs. Hap additionally uses
[Vidvox Hap + bcdec](https://github.com/Vidvox/hap): Vidvox's reference `HapDecode` extracts the
compressed BC7/BC6H texture and [bcdec](https://github.com/iOrange/bcdec) independently decodes those
texture blocks. `—` is a codec with no encoder, so there is nothing for anything to have read. `none`
is an encoder nothing else has looked at. The claim is declared by `[VerifiedBy]` on the encoder type,
carried into `VideoCodecEncoderEntry.VerifiedBy` by the registry generator, and held to what it says by
[`EncoderOracleTests`](https://github.com/Hawkynt/PNGCrushCS/blob/main/Tests/Hawkynt.FileFormats.Video.Tests/EncoderOracleTests.cs).
The ordinary runner asks ffmpeg to decode a frame back at the size that went in; the Hap runner creates
deterministic Hap R and signed/unsigned Hap HDR frames, sends their framing through Vidvox `HapDecode`,
and compares bcdec's reconstructed samples against the source. Either is stronger evidence than this
package's own decoder agreeing with its own encoder, which is what the column exists to stop being
...
Every public and protected member of all 499 types, generated from the built assembly and its XML documentation, is in [REFERENCE.md](https://github.com/Hawkynt/PNGCrushCS/blob/main/Hawkynt.FileFormats.Video/REFERENCE.md).
...
| [Vidvox Hap + bcdec](https://github.com/Vidvox/hap) | Oracle | Hap R/HDR interoperability: Vidvox `HapDecode` validates Hap framing and extracts the texture payload; [bcdec](https://github.com/iOrange/bcdec) independently decodes BC7 and signed/unsigned BC6H samples. CI pins both upstream revisions. |
