...
[`VideoFormatRegistry.CreateDecoder`](https://github.com/Hawkynt/PNGCrushCS/blob/main/Hawkynt.FileFormats.Video/VideoFormatRegistry.cs) builds, 84 of them; `Encode` is
...
| [Intel Indeo Video Interactive 4](https://en.wikipedia.org/wiki/Indeo) | ⚠️ | — | `IV41` entire bar transforms Intel specified and never shipped: wavelet bands, tiling, the slant and Haar transforms in one and two dimensions at both block sizes, all fifteen scans and nine matrices, bidirectional frames including the second frame packed into an intra frame's packet, and the Haar recomposition of a scalable picture. The four discrete cosine transforms, the four-by-four pass-through, a custom scan or quantisation matrix, and any subsampling but YVU9 are refused by name | [FFmpeg `indeo4.c`](https://github.com/FFmpeg/FFmpeg/blob/master/libavcodec/indeo4.c) |
| [Intel Indeo Video Interactive 5](https://en.wikipedia.org/wiki/Indeo) | ⚠️ | — | `IV50` entire: group, picture and band headers, wavelet bands, tiling, the slant transform in one and two dimensions, the untransformed band, run-value maps with their per-band permutations, custom codebooks, half-sample motion compensation and the five-three recomposition of a scalable picture. A password-protected clip, the YV12 picture format, four-by-four luminance blocks and extended transform information are refused by name | [FFmpeg `indeo5.c`](https://github.com/FFmpeg/FFmpeg/blob/master/libavcodec/indeo5.c) |
