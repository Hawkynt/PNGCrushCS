...
| [id Cinematic](https://en.wikipedia.org/wiki/Id_Cinematic) | `Idcin` | `.cin` | ✅ | ✅ | none | [MultimediaWiki CIN](https://wiki.multimedia.cx/index.php/Id_Cinematic) |
| [Westwood VQA](https://en.wikipedia.org/wiki/Westwood_VQA) | `Vqa` | `.vqa` | ✅ | ✅ | none | [MultimediaWiki VQA](https://wiki.multimedia.cx/index.php/Westwood_VQA) |
...
silently misdecodes what it will not read. An `Encode` tick is a lossless or explicitly documented
format-faithful writer; any deliberate reduction the writer performs is called out in its row rather
than hidden behind the tick. Codec-by-codec provenance and measurement notes are in
...
- Encoding is a smaller domain than decoding on purpose: 47 codecs of the 100 read can also be written. Most are lossless; thirteen are not, and each says so in its own row. Twelve codecs — Motion JPEG, DV, Microsoft Video 1, Cinepak, Microsoft's MPEG-4, ASUS V1 and V2, Apple Video, H.261, Hap, Apple ProRes and id RoQ — are intrinsically lossy in the forms written here. The HEVC writer is the thirteenth because this first managed writer converts RGB input to Main-profile 8-bit 4:2:0 before coding the resulting samples losslessly as PCM. What is still not written back includes H.264 and other modern transform codecs.
...
If this project saves you time or money, consider supporting this development:
