...
public sealed class DataTypeTests {
  ...
  public void ColorMode_ValuesMatchPngSpec() {
    Assert.That((byte)ColorMode.Grayscale, Is.EqualTo(0));
    Assert.That((byte)ColorMode.RGB, Is.EqualTo(2));
    Assert.That((byte)ColorMode.Palette, Is.EqualTo(3));
    Assert.That((byte)ColorMode.GrayscaleAlpha, Is.EqualTo(4));
    Assert.That((byte)ColorMode.RGBA, Is.EqualTo(6));
  }

  [Test]
  public void InterlaceMethod_ValuesMatchPngSpec() {
    Assert.That((byte)InterlaceMethod.None, Is.EqualTo(0));
    Assert.That((byte)InterlaceMethod.Adam7, Is.EqualTo(1));
  }

  [Test]
  public void FilterType_ValuesMatchPngSpec() {
    Assert.That((int)FilterType.None, Is.EqualTo(0));
    Assert.That((int)FilterType.Sub, Is.EqualTo(1));
    Assert.That((int)FilterType.Up, Is.EqualTo(2));
    Assert.That((int)FilterType.Average, Is.EqualTo(3));
    Assert.That((int)FilterType.Paeth, Is.EqualTo(4));
  }

  [Test]
  public void FilterType_HasExactlyFiveValues() {
    var values = Enum.GetValues<FilterType>();
  ...
  public void OptimizationCombo_IsReadonlyRecordStruct() {
    ...
    Assert.That(combo.ColorMode, Is.EqualTo(ColorMode.RGB));
    Assert.That(combo.BitDepth, Is.EqualTo(8));
    Assert.That(combo.InterlaceMethod, Is.EqualTo(InterlaceMethod.None));
