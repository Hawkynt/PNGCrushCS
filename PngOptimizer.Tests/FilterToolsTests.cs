...
public sealed class FilterToolsTests {
  ...
  public void ApplyFilter_None_ReturnsSameData() {
    ...
    var result = FilterTools.ApplyFilter(FilterType.None, scanline, ReadOnlySpan<byte>.Empty, 1);
  ...
  public void ApplyFilter_Sub_SubtractsLeftNeighbor() {
    ...
    var result = FilterTools.ApplyFilter(FilterType.Sub, scanline, ReadOnlySpan<byte>.Empty, 1);
  ...
  public void ApplyFilter_Sub_WithMultiBytesPerPixel() {
    ...
    var result = FilterTools.ApplyFilter(FilterType.Sub, scanline, ReadOnlySpan<byte>.Empty, 3);
  ...
  public void ApplyFilter_Up_SubtractsPreviousScanline() {
    ...
    var result = FilterTools.ApplyFilter(FilterType.Up, scanline, previous, 1);
  ...
  public void ApplyFilter_Up_NoPreviousScanline_ReturnsSameData() {
    ...
    var result = FilterTools.ApplyFilter(FilterType.Up, scanline, ReadOnlySpan<byte>.Empty, 1);
    Assert.That(result, Is.EqualTo(scanline));
  }
  ...
  public void ApplyFilter_Average_ComputesCorrectly() {
    ...
    var result = FilterTools.ApplyFilter(FilterType.Average, scanline, previous, 1);
  ...
  public void ApplyFilter_Average_NoPreviousScanline() {
    byte[] scanline = [10, 20, 30];
    var result = FilterTools.ApplyFilter(FilterType.Average, scanline, ReadOnlySpan<byte>.Empty, 1);
    Assert.That(result[0], Is.EqualTo(10));
  ...
  public void ApplyFilter_Paeth_ComputesCorrectly() {
    ...
    var result = FilterTools.ApplyFilter(FilterType.Paeth, scanline, previous, 1);
  ...
  public void ApplyFilter_Paeth_NoPreviousScanline_EqualsSubFilter() {
    byte[] scanline = [10, 20, 30, 40];
    var resultPaeth = FilterTools.ApplyFilter(FilterType.Paeth, scanline, ReadOnlySpan<byte>.Empty, 1);
    var resultSub = FilterTools.ApplyFilter(FilterType.Sub, scanline, ReadOnlySpan<byte>.Empty, 1);
  ...
  public void ApplyFilters_MultiRow_AppliesCorrectFilterPerRow() {
    ...
    FilterType[] filters = [FilterType.None, FilterType.Up, FilterType.Sub];

    var result = FilterTools.ApplyFilters(imageData, filters, 1);

    Assert.That(result.Length, Is.EqualTo(3));
    Assert.That(result[0][0], Is.EqualTo((byte)FilterType.None));
    Assert.That(result[1][0], Is.EqualTo((byte)FilterType.Up));
    Assert.That(result[2][0], Is.EqualTo((byte)FilterType.Sub));
  ...
  [Test]
  public void ApplyFilter_InvalidFilterType_ThrowsArgumentOutOfRange() {
    byte[] scanline = [1, 2, 3];
    Assert.Throws<ArgumentOutOfRangeException>(() =>
      FilterTools.ApplyFilter((FilterType)99, scanline, ReadOnlySpan<byte>.Empty, 1));
  }

  [Test]
  public void ApplyFilter_SingleByteScanline_AllFilters() {
    byte[] scanline = [42];
    foreach (var filterType in Enum.GetValues<FilterType>()) {
  ...
  public void Paeth_LargeScanline_SimdMatchesScalar(int bytesPerPixel) {
    ...
    var actual = FilterTools.ApplyFilter(FilterType.Paeth, scanline, previousScanline, bytesPerPixel);
