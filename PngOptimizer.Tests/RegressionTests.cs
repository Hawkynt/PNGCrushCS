...
public sealed class RegressionTests {
  [Test]
  [TestCase(ColorMode.Palette, 1, 1)]
  [TestCase(ColorMode.Palette, 2, 1)]
  [TestCase(ColorMode.Palette, 4, 1)]
  [TestCase(ColorMode.Palette, 8, 1)]
  [TestCase(ColorMode.Grayscale, 1, 1)]
  [TestCase(ColorMode.Grayscale, 2, 1)]
  [TestCase(ColorMode.Grayscale, 4, 1)]
  [TestCase(ColorMode.Grayscale, 8, 1)]
  [TestCase(ColorMode.GrayscaleAlpha, 8, 2)]
  [TestCase(ColorMode.RGB, 8, 3)]
  [TestCase(ColorMode.RGBA, 8, 4)]
  public void GetFilterStride_NeverReturnsZero(ColorMode mode, int bitDepth, int expectedStride) {
  ...
  public void GetSamplesPerPixel_AllColorModes() {
    Assert.That(PngOptimizer.GetSamplesPerPixel(ColorMode.Grayscale), Is.EqualTo(1));
    Assert.That(PngOptimizer.GetSamplesPerPixel(ColorMode.GrayscaleAlpha), Is.EqualTo(2));
    Assert.That(PngOptimizer.GetSamplesPerPixel(ColorMode.RGB), Is.EqualTo(3));
    Assert.That(PngOptimizer.GetSamplesPerPixel(ColorMode.RGBA), Is.EqualTo(4));
    Assert.That(PngOptimizer.GetSamplesPerPixel(ColorMode.Palette), Is.EqualTo(1));
  }
