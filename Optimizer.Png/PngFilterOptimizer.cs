...
public sealed class PngFilterOptimizer(
  ...
  public PngFilterType[] OptimizeFilters(FilterStrategy strategy) {
  ...
  private PngFilterType[] OptimizeWeightedContinuity() {
  ...
  private PngFilterType[] OptimizeSingleFilter() {
    if ((isPalette || isGrayscale) && bitDepth < 8)
      return Enumerable.Repeat(PngFilterType.None, height).ToArray();
  ...
  private PngFilterType[] OptimizeScanlineAdaptive() {
    var selectedFilters = new PngFilterType[height];
    for (var y = 0; y < height; ++y) {
      var scanline = imageData[y];
      var previousScanline = y > 0 ? imageData[y - 1] : null;
  ...
  private PngFilterType[] OptimizeBruteForceAdaptive() {
    if ((isPalette || isGrayscale) && bitDepth < 8)
      return Enumerable.Repeat(PngFilterType.None, height).ToArray();
    ...
    for (var y = 0; y < height; ++y) {
      ...
      if (isAmbiguous) {
        ...
        var bestCandidate = (PngFilterType)best;

        foreach (var candidateIdx in new[] { best, secondBest }) {
          var candidate = (PngFilterType)candidateIdx;
        ...
        selectedFilters[y] = (PngFilterType)best;
      }
    }
  ...
  private PngFilterType[] OptimizeBruteForce() {
    if ((isPalette || isGrayscale) && bitDepth < 8)
      return Enumerable.Repeat(PngFilterType.None, height).ToArray();
