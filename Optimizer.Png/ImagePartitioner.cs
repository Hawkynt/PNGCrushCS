...
public sealed partial class ImagePartitioner(
  ...
  public (PngFilterType[] filters, byte[][] filteredData) OptimizePartitions() {
    if ((isPalette || isGrayscale) && bitDepth < 8)
      return this.OptimizeWithSingleFilter(PngFilterType.None);

    return this.OptimizeWithSmartPartitioning();
  }

  /// <summary>Optimizes using a single filter for the entire image</summary>
  private (PngFilterType[] filters, byte[][] filteredData) OptimizeWithSingleFilter(PngFilterType filterType) {
  ...
  private (PngFilterType[] filters, byte[][] filteredData) OptimizeWithSmartPartitioning() {
  ...
  private PngFilterType[] OptimizeFiltersWithSmartPartitioning() {
    var stats = new FilterStatsPerRow[height][];
    for (var y = 0; y < height; ++y)
      stats[y] = this.CalculateAllFilterScores(y);

    var filters = new PngFilterType[height];
    var currentFilter = PngFilterType.None;
    for (var y = 0; y < height; ++y) {
    ...
    static PngFilterType FindBestFilter(FilterStatsPerRow[] scores) {
