...
/// <summary>Reads GFA Paint files from bytes, streams, or file paths.</summary>
public static class GfaPaintReader {

  public static GfaPaintFile FromFile(FileInfo file) {
    ArgumentNullException.ThrowIfNull(file);
    if (!file.Exists)
      throw new FileNotFoundException("GFA Paint file not found.", file.FullName);

    return FromBytes(File.ReadAllBytes(file.FullName));
  }

  public static GfaPaintFile FromStream(Stream stream) {
  ...
  public static GfaPaintFile FromBytes(byte[] data) {
    ...
    var pixelData = new byte[32000];
    data.AsSpan(GfaPaintHeader.StructSize, 32000).CopyTo(pixelData.AsSpan(0));
