...
public readonly record struct GfaPaintFile : IImageFormatReader<GfaPaintFile>, IImageToRawImage<GfaPaintFile>, IImageFromRawImage<GfaPaintFile>, IImageFormatWriter<GfaPaintFile> {
  ...
  public static GfaPaintFile FromRawImage(RawImage image) {
    ArgumentNullException.ThrowIfNull(image);
    if (image.Format != PixelFormat.Indexed8)
      throw new ArgumentException("RawImage must use PixelFormat.Indexed8.", nameof(image));
    if (image.Width != 320)
      throw new ArgumentException("GFA Paint images must be exactly 320 pixels wide.", nameof(image));
    if (image.Height != 200)
      throw new ArgumentException("GFA Paint images must be exactly 200 pixels tall.", nameof(image));
