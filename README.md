...
- **Read-only formats** — of 847 registered formats all 847 decode and 785 can encode an arbitrary image; the other 62 parse a file they read without being able to author one from pixel data. Several are read-only on purpose rather than for want of effort: PDF and PE-resource extraction are one-way by nature, HAM-E and DCTV encode colour by a method never published, an embedded preview is somebody else's file rather than a picture of its own, and AVIF and HEIF have decoders here but no encoder — a container holding uncoded pixels is not one of those formats, whatever it is named. Run `Decode --readonly` for the current list; the table in [`Formats.md`](Formats.md) is maintained by hand and drifts behind it.
- **The JPEG encoder is lossier than it should be, and its files are larger** — measured, not suspected. Against ImageMagick at the same stated quality of 90, a 320 by 200 ramp comes out at three times the mean error in a file a third larger; a 37 by 11 one at twenty-two times the error, again in a larger file. Worse on both counts at once is not a trade-off, so this is a fault rather than a setting. Our *decoder* matches ImageMagick exactly on ImageMagick's own files, and the block builder replicates edge pixels correctly, so neither of those is the cause. It is worst where the picture is small against the eight-by-eight grid, which is where to look first.
