...
- Some optimizer paths use `System.Drawing.Common` and are Windows-specific; the public image-format package itself is cross-platform.
- Optimizer pipelines primarily work in 8-bit pixel space even though many readers/writers preserve 16-bit scientific/HDR data.
- Some modern codecs intentionally support subsets. See package-specific READMEs and `codec-coverage.md` for the exact scope.
- Registry presence does not mean equivalent conformance maturity for every historical format. Writers are withheld when no trustworthy external validator exists.
