...
public static class GfaRaytraceReader {
  ...
  public static GfaRaytraceFile FromStream(Stream stream) {
  ...
  public static GfaRaytraceFile FromSpan(ReadOnlySpan<byte> data) {
    ...
    var available = Math.Min(pixelBytes, data.Length - GfaRaytraceFile.HeaderSize);
    if (available > 0)
      data.Slice(GfaRaytraceFile.HeaderSize, available).CopyTo(pixelData.AsSpan(0));
  ...
  public static GfaRaytraceFile FromBytes(byte[] data) {
    ...
    var available = Math.Min(pixelBytes, data.Length - GfaRaytraceFile.HeaderSize);
    if (available > 0)
      data.AsSpan(GfaRaytraceFile.HeaderSize, available).CopyTo(pixelData.AsSpan(0));
