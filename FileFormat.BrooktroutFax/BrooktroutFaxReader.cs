...
public static class BrooktroutFaxReader {
  ...
  public static BrooktroutFaxFile FromSpan(ReadOnlySpan<byte> data) {
    ...
    var available = Math.Min(pixelBytes, data.Length - BrooktroutFaxFile.HeaderSize);
    if (available > 0)
      data.Slice(BrooktroutFaxFile.HeaderSize, available).CopyTo(pixelData.AsSpan(0));
  ...
  public static BrooktroutFaxFile FromBytes(byte[] data) {
    ...
    var available = Math.Min(pixelBytes, data.Length - BrooktroutFaxFile.HeaderSize);
    if (available > 0)
      data.AsSpan(BrooktroutFaxFile.HeaderSize, available).CopyTo(pixelData.AsSpan(0));
