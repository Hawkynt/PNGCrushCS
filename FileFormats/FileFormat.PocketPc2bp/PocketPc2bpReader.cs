...
public static class PocketPc2bpReader {
  ...
  public static PocketPc2bpFile FromSpan(ReadOnlySpan<byte> data) {
    ...
    var available = Math.Min(pixelBytes, data.Length - PocketPc2bpFile.HeaderSize);
    if (available > 0)
      data.Slice(PocketPc2bpFile.HeaderSize, available).CopyTo(pixelData.AsSpan(0));
  ...
  public static PocketPc2bpFile FromBytes(byte[] data) {
    ...
    var available = Math.Min(pixelBytes, data.Length - PocketPc2bpFile.HeaderSize);
    if (available > 0)
      data.AsSpan(PocketPc2bpFile.HeaderSize, available).CopyTo(pixelData.AsSpan(0));
