...
public readonly record struct QuakeLmpFile : IImageFormatReader<QuakeLmpFile>, IImageToRawImage<QuakeLmpFile>, IImageFromRawImage<QuakeLmpFile>, IImageFormatWriter<QuakeLmpFile> {
  ...
  /// <summary>Default 256-entry RGB palette derived from the canonical Quake fixed palette.</summary>
  private static readonly byte[] _DefaultPalette = _BuildDefaultPalette();

  private static byte[] _BuildDefaultPalette() {
    var src = _FixedPalettes[0].ToPackedRgb();
    var p = new byte[256 * 3];
    Array.Copy(src, 0, p, 0, Math.Min(src.Length, p.Length));
    return p;
  }
