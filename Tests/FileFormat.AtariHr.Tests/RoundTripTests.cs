...
public sealed class RoundTripTests {
  ...
  public void AllZeros_RoundTrip() {
    var original = new AtariHrFile { RawData = new byte[7680] };

    var bytes = AtariHrWriter.ToBytes(original);
    var restored = AtariHrReader.FromBytes(bytes);
  ...
  public void AllOnes_RoundTrip() {
    ...
    var original = new AtariHrFile { RawData = rawData };
  ...
  public void ViaFile_RoundTrip() {
    ...
    try {
      ...
      var restored = AtariHrReader.FromFile(new FileInfo(tempPath));
  ...
  public void ViaRawImage_RoundTrip() {
    ...
    var raw = AtariHrFile.ToRawImage(original);
    var restored = AtariHrFile.FromRawImage(raw);
  ...
  public void SpecificRow_PixelMapping_MsbFirst() {
    ...
    rawData[0] = 0x01;
    var original2 = new AtariHrFile { RawData = rawData };
    var raw2 = AtariHrFile.ToRawImage(original2);
