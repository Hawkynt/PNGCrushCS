...
public sealed class RoundTripTests {
  ...
  public void RoundTrip_Grayscale() {
    ...
    var bytes = PtifWriter.ToBytes(original);
    var restored = PtifReader.FromBytes(bytes);
  ...
  public void RoundTrip_ViaFile() {
    ...
    var tempPath = Path.Combine(Path.GetTempPath(), Guid.NewGuid() + ".ptif");
    try {
      var bytes = PtifWriter.ToBytes(original);
      File.WriteAllBytes(tempPath, bytes);
      var restored = PtifReader.FromFile(new FileInfo(tempPath));
  ...
  public void RoundTrip_ViaStream() {
    ...
    var restored = PtifReader.FromStream(ms);
  ...
  public void RoundTrip_RawImage_Grayscale() {
    ...
    var ptif = PtifFile.FromRawImage(rawImage);
    var raw = PtifFile.ToRawImage(ptif);
  ...
  [Test]
  [Category("Integration")]
  public void RoundTrip_RawImage_Rgba() {
    ...
    Assert.That(raw.Height, Is.EqualTo(2));
    Assert.That(raw.Format, Is.EqualTo(FileFormat.Core.PixelFormat.Rgba32));
    Assert.That(raw.PixelData, Is.EqualTo(pixelData));
  }
}
