...
public sealed class RainbowPainterReaderTests {
  ...
  public void FromFile_Null_ThrowsArgumentNullException() {
    Assert.Throws<ArgumentNullException>(() => RainbowPainterReader.FromFile(null!));
  }

  [Test]
  [Category("Unit")]
  public void FromFile_Missing_ThrowsFileNotFoundException() {
    var missing = new FileInfo(Path.Combine(Path.GetTempPath(), Guid.NewGuid() + ".rp"));
    Assert.Throws<FileNotFoundException>(() => RainbowPainterReader.FromFile(missing));
  }

  [Test]
  [Category("Unit")]
  public void FromStream_Null_ThrowsArgumentNullException() {
    Assert.Throws<ArgumentNullException>(() => RainbowPainterReader.FromStream(null!));
  }

  [Test]
  [Category("Unit")]
  public void FromBytes_TooSmall_ThrowsInvalidDataException() {
    Assert.Throws<InvalidDataException>(() => RainbowPainterReader.FromBytes(new byte[100]));
  }

  [Test]
  [Category("Unit")]
  public void FromBytes_WrongSize_ThrowsInvalidDataException() {
    Assert.Throws<InvalidDataException>(() => RainbowPainterReader.FromBytes(new byte[10004]));
  }
  ...
  public void FromStream_ValidParsesCorrectly() {
    ...
    var result = RainbowPainterReader.FromStream(ms);
  ...
  public void RoundTrip_AllFieldsPreserved() {
    ...
    var original = new RainbowPainterFile {
      LoadAddress = 0x6000,
    ...
    var bytes = RainbowPainterWriter.ToBytes(original);
    var restored = RainbowPainterReader.FromBytes(bytes);
