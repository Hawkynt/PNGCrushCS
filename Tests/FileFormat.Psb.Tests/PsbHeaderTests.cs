...
[TestFixture]
public sealed class PsbHeaderTests {

  [Test]
  [Category("Unit")]
  public void RoundTrip_PreservesAllFields() {
    var original = new PsbHeader(
    ...
    Span<byte> buffer = stackalloc byte[PsbHeader.StructSize];
    original.WriteTo(buffer);
    var parsed = PsbHeader.ReadFrom(buffer);
  ...
  public void ReadFrom_ParsesKnownValues() {
    var data = new byte[PsbHeader.StructSize];
    data[0] = (byte)'8';
    data[1] = (byte)'B';
    data[2] = (byte)'P';
    data[3] = (byte)'S';
    BinaryPrimitives.WriteInt16BigEndian(data.AsSpan(4), 2);       // version (PSB)
  ...
  public void GetFieldMap_CoversFullStructSize() {
    var map = PsbHeader.GetFieldMap();
    var totalSize = map.Sum(f => f.Size);
    Assert.That(totalSize, Is.EqualTo(PsbHeader.StructSize));
  }
  ...
  public void StructSize_Is26() {
    Assert.That(PsbHeader.StructSize, Is.EqualTo(26));
  }
}
