...
public sealed class RoundTripTests {
  ...
  public void RoundTrip_ViaFile() {
    ...
    var tempPath = Path.Combine(Path.GetTempPath(), Guid.NewGuid() + ".dpan");
    try {
      var bytes = IffDpanWriter.ToBytes(original);
      File.WriteAllBytes(tempPath, bytes);

      var restored = IffDpanReader.FromFile(new FileInfo(tempPath));
  ...
  public void RoundTrip_ViaRawImage() {
    ...
    var raw = IffDpanFile.ToRawImage(original);
  ...
  public void RoundTrip_AllZeros_PreservesData() {
    var rawData = new byte[IffDpanFile.MinFileSize];
