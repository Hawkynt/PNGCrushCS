...
[TestFixture]
public sealed class C128MultiFromRawImageTests {
  ...
  public void EncodeThenDecode_ReproducesAPictureTheFormatCanHold() {
    ...
    var decoded = C128MultiFile.ToRawImage(C128MultiFile.FromRawImage(source));
  ...
  public void ADifferentlySizedPictureIsScaledRatherThanRefused() {
    ...
    var decoded = C128MultiFile.ToRawImage(C128MultiFile.FromRawImage(_Stripes(96, 72)));
  ...
  public void WhatIsEncodedSurvivesTheWriterAndTheReader() {
    var file = C128MultiFile.FromRawImage(_Stripes(160, 200));
    var restored = C128MultiReader.FromBytes(C128MultiWriter.ToBytes(file));

    Assert.That(_Rgb(C128MultiFile.ToRawImage(restored)), Is.EqualTo(_Rgb(C128MultiFile.ToRawImage(file))));
  }
}
