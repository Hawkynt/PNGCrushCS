...
[TestFixture]
public sealed class HiresInterlaceFeniksFromRawImageTests {
  ...
  /// </summary>
  /// <remarks>
  /// Two colours to a cell and to every raster line of one, which is inside what a hires interlace pair can hold,
  ...
  public void EncodeThenDecode_ReproducesAPictureTheFormatCanHold() {
    ...
    var decoded = HiresInterlaceFeniksFile.ToRawImage(HiresInterlaceFeniksFile.FromRawImage(source));
  ...
  public void ADifferentlySizedPictureIsScaledRatherThanRefused() {
    ...
    var decoded = HiresInterlaceFeniksFile.ToRawImage(HiresInterlaceFeniksFile.FromRawImage(_Stripes(96, 72)));
  ...
  public void WhatIsEncodedSurvivesTheWriterAndTheReader() {
    var file = HiresInterlaceFeniksFile.FromRawImage(_Stripes(320, 200));
    var restored = HiresInterlaceFeniksReader.FromBytes(HiresInterlaceFeniksWriter.ToBytes(file));

    Assert.That(_Rgb(HiresInterlaceFeniksFile.ToRawImage(restored)), Is.EqualTo(_Rgb(HiresInterlaceFeniksFile.ToRawImage(file))));
  }
}
