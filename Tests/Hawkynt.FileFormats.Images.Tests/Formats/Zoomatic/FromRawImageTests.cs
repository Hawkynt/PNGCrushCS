...
[TestFixture]
public sealed class ZoomaticFromRawImageTests {
  ...
  public void EncodeThenDecode_ReproducesAPictureTheFormatCanHold() {
    ...
    var decoded = ZoomaticFile.ToRawImage(ZoomaticFile.FromRawImage(source));
  ...
  public void ADifferentlySizedPictureIsScaledRatherThanRefused() {
    ...
    var decoded = ZoomaticFile.ToRawImage(ZoomaticFile.FromRawImage(_Stripes(96, 72)));
  ...
  public void FromRawImage_Null_Throws()
    => Assert.Throws<ArgumentNullException>(() => ZoomaticFile.FromRawImage(null!));

  [Test]
  [Category("Unit")]
  public void WhatIsEncodedSurvivesTheWriterAndTheReader() {
    var file = ZoomaticFile.FromRawImage(_Stripes(160, 200));
    var restored = ZoomaticReader.FromBytes(ZoomaticWriter.ToBytes(file));

    Assert.That(_Rgb(ZoomaticFile.ToRawImage(restored)), Is.EqualTo(_Rgb(ZoomaticFile.ToRawImage(file))));
  }
}
