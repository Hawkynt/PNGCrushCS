...
[TestFixture]
public sealed class FlimaticFromRawImageTests {
  ...
  public void EncodeThenDecode_ReproducesAPictureTheFormatCanHold() {
    ...
    var decoded = FlimaticFile.ToRawImage(FlimaticFile.FromRawImage(source));
  ...
  public void ADifferentlySizedPictureIsScaledRatherThanRefused() {
    ...
    var decoded = FlimaticFile.ToRawImage(FlimaticFile.FromRawImage(_Stripes(96, 72)));
  ...
  public void WhatIsEncodedSurvivesTheWriterAndTheReader() {
    var file = FlimaticFile.FromRawImage(_Stripes(160, 200));
    var restored = FlimaticReader.FromBytes(FlimaticWriter.ToBytes(file));

    Assert.That(_Rgb(FlimaticFile.ToRawImage(restored)), Is.EqualTo(_Rgb(FlimaticFile.ToRawImage(file))));
  }
}
