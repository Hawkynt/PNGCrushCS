...
public sealed class RoundTripTests {
  ...
  public void RoundTrip_AllFieldsPreserved() {
    ...
    var original = new HinterGrundBildFile {
      LoadAddress = 0x4000,
    ...
    var bytes = HinterGrundBildWriter.ToBytes(original);
    var restored = HinterGrundBildReader.FromBytes(bytes);
  ...
  public void RoundTrip_ViaFile_PreservesData() {
    ...
    var tmpPath = Path.Combine(Path.GetTempPath(), Guid.NewGuid() + ".hgb");
    try {
      var bytes = HinterGrundBildWriter.ToBytes(original);
      File.WriteAllBytes(tmpPath, bytes);
      var restored = HinterGrundBildReader.FromFile(new FileInfo(tmpPath));
  ...
  public void RoundTrip_ToRawImage_ProducesCorrectDimensions() {
    ...
    var raw = HinterGrundBildFile.ToRawImage(original);
