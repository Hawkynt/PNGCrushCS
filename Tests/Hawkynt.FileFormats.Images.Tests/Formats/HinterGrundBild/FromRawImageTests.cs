...
[TestFixture]
public sealed class HinterGrundBildFromRawImageTests {
  ...
  public void EncodeThenDecode_ReproducesAPictureTheFormatCanHold() {
    ...
    var decoded = HinterGrundBildFile.ToRawImage(HinterGrundBildFile.FromRawImage(source));
  ...
  public void ADifferentlySizedPictureIsScaledRatherThanRefused() {
    ...
    var decoded = HinterGrundBildFile.ToRawImage(HinterGrundBildFile.FromRawImage(_Stripes(96, 72)));
  ...
  public void FromRawImage_Null_Throws()
    => Assert.Throws<ArgumentNullException>(() => HinterGrundBildFile.FromRawImage(null!));

  [Test]
  [Category("Unit")]
  public void WhatIsEncodedSurvivesTheWriterAndTheReader() {
    var file = HinterGrundBildFile.FromRawImage(_Stripes(160, 200));
    var restored = HinterGrundBildReader.FromBytes(HinterGrundBildWriter.ToBytes(file));

    Assert.That(_Rgb(HinterGrundBildFile.ToRawImage(restored)), Is.EqualTo(_Rgb(HinterGrundBildFile.ToRawImage(file))));
  }
}
