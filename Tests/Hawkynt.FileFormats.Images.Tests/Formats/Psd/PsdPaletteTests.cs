...
/// </remarks>
[TestFixture]
public sealed class PsdPaletteTests
{
    ...
    private static PsdFile Indexed(byte[] indices, byte[]? palette) => new()
    {
        Width = 4,
        Height = 1,
        Channels = 1,
    ...
    {
        var rgb = PsdFile.ToRawImage(Indexed([0, 1, 2, 3], PlanarTable())).ToRgb24();
    ...
    public void Write_AlwaysEmitsAFullTable()
    {
        ...
        var file = PsdFile.FromRawImage(image);
    ...
    public void Write_PutsTheTableInTheColourModeSection()
    {
        ...
        var bytes = PsdWriter.ToBytes(PsdFile.FromRawImage(image));
    ...
    public void RoundTrip_KeepsTheColours()
    {
        ...
        var back = PsdFile.ToRawImage(PsdReader.FromBytes(PsdWriter.ToBytes(PsdFile.FromRawImage(image)))).ToRgb24();
