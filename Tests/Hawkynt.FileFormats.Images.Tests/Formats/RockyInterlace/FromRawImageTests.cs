...
[TestFixture]
public sealed class RockyInterlaceFromRawImageTests {
  ...
  public void EncodeThenDecode_ReproducesAPictureTheFormatCanHold() {
    ...
    var decoded = RockyInterlaceFile.ToRawImage(RockyInterlaceFile.FromRawImage(source));
  ...
  public void ADifferentlySizedPictureIsScaledRatherThanRefused() {
    ...
    var decoded = RockyInterlaceFile.ToRawImage(RockyInterlaceFile.FromRawImage(_Stripes(96, 72)));
  ...
  public void WhatIsEncodedSurvivesTheWriterAndTheReader() {
    var file = RockyInterlaceFile.FromRawImage(_Stripes(320, 200));
    var restored = RockyInterlaceReader.FromBytes(RockyInterlaceWriter.ToBytes(file));

    Assert.That(_Rgb(RockyInterlaceFile.ToRawImage(restored)), Is.EqualTo(_Rgb(RockyInterlaceFile.ToRawImage(file))));
  }
}
