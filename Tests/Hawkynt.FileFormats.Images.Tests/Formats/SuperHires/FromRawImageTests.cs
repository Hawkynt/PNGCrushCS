...
public sealed class FromRawImageTests {
  ...
  internal static RawImage HiresImage(int width, int height) {
    var rgb = new byte[width * height * 3];

    for (var y = 0; y < height; ++y)
    for (var x = 0; x < width; ++x) {
      var cell = y / 8 * (width / 8) + x / 8;
      var index = ((x + y) & 1) == 0 ? cell % 16 : (cell * 7 + 3) % 16;
  ...
  public void RoundTrip_APictureTheScreenCanHold_IsExact() {
    var source = HiresImage(SuperHiresFile.ImageWidth, SuperHiresFile.ImageHeight);

    var bytes = SuperHiresWriter.ToBytes(_Encode<SuperHiresFile>(source));
    var decoded = SuperHiresFile.ToRawImage(SuperHiresReader.FromBytes(bytes));
  ...
  public void FromRawImage_SamplesAPictureOfAnotherSize() {
    var file = _Encode<SuperHiresFile>(HiresImage(160, 100));

    Assert.Multiple(() => {
      Assert.That(file.BitmapData1, Has.Length.EqualTo(SuperHiresFile.BitmapDataSize));
      Assert.That(file.BitmapData2, Is.EqualTo(file.BitmapData1));
      Assert.That(SuperHiresWriter.ToBytes(file), Has.Length.EqualTo(SuperHiresFile.ExpectedFileSize));
    });
  }
