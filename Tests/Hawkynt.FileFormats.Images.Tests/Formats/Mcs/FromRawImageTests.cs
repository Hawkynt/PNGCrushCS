...
[TestFixture]
public sealed class McsFromRawImageTests {
  ...
  public void EncodeThenDecode_ReproducesAPictureTheFormatCanHold() {
    ...
    var decoded = McsFile.ToRawImage(McsFile.FromRawImage(source));
  ...
  public void ADifferentlySizedPictureIsScaledRatherThanRefused() {
    ...
    var decoded = McsFile.ToRawImage(McsFile.FromRawImage(_Stripes(96, 72)));
  ...
  public void FromRawImage_Null_Throws()
    => Assert.Throws<ArgumentNullException>(() => McsFile.FromRawImage(null!));

  [Test]
  [Category("Unit")]
  public void WhatIsEncodedSurvivesTheWriterAndTheReader() {
    var file = McsFile.FromRawImage(_Stripes(160, 200));
    var restored = McsReader.FromBytes(McsWriter.ToBytes(file));

    Assert.That(_Rgb(McsFile.ToRawImage(restored)), Is.EqualTo(_Rgb(McsFile.ToRawImage(file))));
  }
}
