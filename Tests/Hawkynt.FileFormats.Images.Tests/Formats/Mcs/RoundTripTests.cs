...
public sealed class RoundTripTests {
  ...
  public void RoundTrip_AllFieldsPreserved() {
    ...
    var bytes = McsWriter.ToBytes(original);
    var restored = McsReader.FromBytes(bytes);
  ...
  public void RoundTrip_ViaFile_PreservesData() {
    ...
    var tmpPath = Path.Combine(Path.GetTempPath(), Guid.NewGuid() + ".mcs");
    try {
      var bytes = McsWriter.ToBytes(original);
      File.WriteAllBytes(tmpPath, bytes);
      var restored = McsReader.FromFile(new FileInfo(tmpPath));
  ...
  public void RoundTrip_ToRawImage_ProducesCorrectDimensions() {
    ...
    var raw = McsFile.ToRawImage(original);
