...
[TestFixture]
public sealed class CfliDesignerFromRawImageTests {
  ...
  /// </summary>
  /// <remarks>
  /// Two colours to a cell and to every raster line of one, which is inside what a colour-only FLI screen can hold,
  ...
  public void EncodeThenDecode_ReproducesAPictureTheFormatCanHold() {
    var source = _Stripes(296, 200);
    var decoded = CfliDesignerFile.ToRawImage(CfliDesignerFile.FromRawImage(source));
  ...
  public void ADifferentlySizedPictureIsScaledRatherThanRefused() {
    ...
    var decoded = CfliDesignerFile.ToRawImage(CfliDesignerFile.FromRawImage(_Stripes(96, 72)));
  ...
  public void WhatIsEncodedSurvivesTheWriterAndTheReader() {
    var file = CfliDesignerFile.FromRawImage(_Stripes(296, 200));
    var restored = CfliDesignerReader.FromBytes(CfliDesignerWriter.ToBytes(file));

    Assert.That(_Rgb(CfliDesignerFile.ToRawImage(restored)), Is.EqualTo(_Rgb(CfliDesignerFile.ToRawImage(file))));
  }
}
