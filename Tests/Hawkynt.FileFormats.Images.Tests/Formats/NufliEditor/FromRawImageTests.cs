...
[TestFixture]
public sealed class NufliEditorFromRawImageTests {
  ...
  public void EncodeThenDecode_ReproducesAPictureTheFormatCanHold() {
    ...
    var decoded = NufliEditorFile.ToRawImage(NufliEditorFile.FromRawImage(source));
  ...
  public void ADifferentlySizedPictureIsScaledRatherThanRefused() {
    ...
    var decoded = NufliEditorFile.ToRawImage(NufliEditorFile.FromRawImage(_Stripes(96, 72)));
  ...
  public void WhatIsEncodedSurvivesTheWriterAndTheReader() {
    var file = NufliEditorFile.FromRawImage(_Stripes(160, 200));
    var restored = NufliEditorReader.FromBytes(NufliEditorWriter.ToBytes(file));

    Assert.That(_Rgb(NufliEditorFile.ToRawImage(restored)), Is.EqualTo(_Rgb(NufliEditorFile.ToRawImage(file))));
  }
}
