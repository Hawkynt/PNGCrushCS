...
[TestFixture]
public sealed class XFliEditorFromRawImageTests {
  ...
  public void EncodeThenDecode_ReproducesAPictureTheFormatCanHold() {
    ...
    var decoded = XFliEditorFile.ToRawImage(XFliEditorFile.FromRawImage(source));
  ...
  public void ADifferentlySizedPictureIsScaledRatherThanRefused() {
    ...
    var decoded = XFliEditorFile.ToRawImage(XFliEditorFile.FromRawImage(_Stripes(96, 72)));
  ...
  public void FromRawImage_Null_Throws()
    => Assert.Throws<ArgumentNullException>(() => XFliEditorFile.FromRawImage(null!));

  [Test]
  [Category("Unit")]
  public void WhatIsEncodedSurvivesTheWriterAndTheReader() {
    var file = XFliEditorFile.FromRawImage(_Stripes(160, 200));
    var restored = XFliEditorReader.FromBytes(XFliEditorWriter.ToBytes(file));

    Assert.That(_Rgb(XFliEditorFile.ToRawImage(restored)), Is.EqualTo(_Rgb(XFliEditorFile.ToRawImage(file))));
  }
}
