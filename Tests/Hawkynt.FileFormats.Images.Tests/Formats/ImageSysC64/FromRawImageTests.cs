...
[TestFixture]
public sealed class ImageSysC64FromRawImageTests {
  ...
  public void EncodeThenDecode_ReproducesAPictureTheFormatCanHold() {
    ...
    var decoded = ImageSysC64File.ToRawImage(ImageSysC64File.FromRawImage(source));
  ...
  public void ADifferentlySizedPictureIsScaledRatherThanRefused() {
    ...
    var decoded = ImageSysC64File.ToRawImage(ImageSysC64File.FromRawImage(_Stripes(96, 72)));
  ...
  public void FromRawImage_Null_Throws()
    => Assert.Throws<ArgumentNullException>(() => ImageSysC64File.FromRawImage(null!));

  [Test]
  [Category("Unit")]
  public void WhatIsEncodedSurvivesTheWriterAndTheReader() {
    var file = ImageSysC64File.FromRawImage(_Stripes(160, 200));
    var restored = ImageSysC64Reader.FromBytes(ImageSysC64Writer.ToBytes(file));

    Assert.That(_Rgb(ImageSysC64File.ToRawImage(restored)), Is.EqualTo(_Rgb(ImageSysC64File.ToRawImage(file))));
  }
}
