...
public sealed class RoundTripTests {
  ...
  public void RoundTrip_DataPreserved() {
    ...
    var original = new IffDctvFile { Width = 320, Height = 200, RawData = rawData };
  ...
  public void RoundTrip_ViaFile() {
    ...
    var original = new IffDctvFile { RawData = rawData };
    var tempPath = Path.Combine(Path.GetTempPath(), Guid.NewGuid() + ".dctv");
    try {
      var bytes = IffDctvWriter.ToBytes(original);
      File.WriteAllBytes(tempPath, bytes);

      var restored = IffDctvReader.FromFile(new FileInfo(tempPath));
