...
[TestFixture]
public sealed class MultiPainterFromRawImageTests {
  ...
  public void EncodeThenDecode_ReproducesAPictureTheFormatCanHold() {
    ...
    var decoded = MultiPainterFile.ToRawImage(MultiPainterFile.FromRawImage(source));
  ...
  public void ADifferentlySizedPictureIsScaledRatherThanRefused() {
    ...
    var decoded = MultiPainterFile.ToRawImage(MultiPainterFile.FromRawImage(_Stripes(96, 72)));
  ...
  public void FromRawImage_Null_Throws()
    => Assert.Throws<ArgumentNullException>(() => MultiPainterFile.FromRawImage(null!));

  [Test]
  [Category("Unit")]
  public void WhatIsEncodedSurvivesTheWriterAndTheReader() {
    var file = MultiPainterFile.FromRawImage(_Stripes(160, 200));
    var restored = MultiPainterReader.FromBytes(MultiPainterWriter.ToBytes(file));

    Assert.That(_Rgb(MultiPainterFile.ToRawImage(restored)), Is.EqualTo(_Rgb(MultiPainterFile.ToRawImage(file))));
  }
}
