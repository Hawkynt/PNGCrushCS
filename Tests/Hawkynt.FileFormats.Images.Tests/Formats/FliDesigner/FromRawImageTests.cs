...
[TestFixture]
public sealed class FliDesignerFromRawImageTests {
  ...
  public void EncodeThenDecode_ReproducesAPictureTheFormatCanHold() {
    ...
    var decoded = FliDesignerFile.ToRawImage(FliDesignerFile.FromRawImage(source));
  ...
  public void ADifferentlySizedPictureIsScaledRatherThanRefused() {
    ...
    var decoded = FliDesignerFile.ToRawImage(FliDesignerFile.FromRawImage(_Stripes(96, 72)));
  ...
  public void WhatIsEncodedSurvivesTheWriterAndTheReader() {
    var file = FliDesignerFile.FromRawImage(_Stripes(160, 200));
    var restored = FliDesignerReader.FromBytes(FliDesignerWriter.ToBytes(file));

    Assert.That(_Rgb(FliDesignerFile.ToRawImage(restored)), Is.EqualTo(_Rgb(FliDesignerFile.ToRawImage(file))));
  }
}
