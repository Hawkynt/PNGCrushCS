...
[TestFixture]
public sealed class FliEditorFromRawImageTests {
  ...
  public void EncodeThenDecode_ReproducesAPictureTheFormatCanHold() {
    ...
    var decoded = FliEditorFile.ToRawImage(FliEditorFile.FromRawImage(source));
  ...
  public void ADifferentlySizedPictureIsScaledRatherThanRefused() {
    ...
    var decoded = FliEditorFile.ToRawImage(FliEditorFile.FromRawImage(_Stripes(96, 72)));
  ...
  public void WhatIsEncodedSurvivesTheWriterAndTheReader() {
    var file = FliEditorFile.FromRawImage(_Stripes(160, 200));
    var restored = FliEditorReader.FromBytes(FliEditorWriter.ToBytes(file));

    Assert.That(_Rgb(FliEditorFile.ToRawImage(restored)), Is.EqualTo(_Rgb(FliEditorFile.ToRawImage(file))));
  }
}
