...
[TestFixture]
public sealed class TurboViewFileFromRawImageTests {
  ...
  public void EncodeThenDecode_ReproducesAPictureTheFormatCanHold() {
    ...
    var decoded = TurboViewFile.ToRawImage(TurboViewFile.FromRawImage(source));
  ...
  public void ADifferentlySizedPictureIsScaledRatherThanRefused() {
    ...
    var decoded = TurboViewFile.ToRawImage(TurboViewFile.FromRawImage(_Source(96, 72)));
  ...
  public void FromRawImage_Null_Throws()
    => Assert.Throws<ArgumentNullException>(() => TurboViewFile.FromRawImage(null!));

  [Test]
  [Category("Unit")]
  public void WhatIsEncodedSurvivesTheWriterAndTheReader() {
    var file = TurboViewFile.FromRawImage(_Source(320, 200));
    var restored = TurboViewReader.FromBytes(TurboViewWriter.ToBytes(file));

    Assert.That(_Rgb(TurboViewFile.ToRawImage(restored)), Is.EqualTo(_Rgb(TurboViewFile.ToRawImage(file))));
  }
}
