...
[TestFixture]
public sealed class MuifliEditorFromRawImageTests {
  ...
  /// </summary>
  /// <remarks>
  /// Two colours to a cell and to every raster line of one, which is inside what a multicolour FLI interlace pair can hold,
  ...
  public void EncodeThenDecode_ReproducesAPictureTheFormatCanHold() {
    ...
    var decoded = MuifliEditorFile.ToRawImage(MuifliEditorFile.FromRawImage(source));
  ...
  public void ADifferentlySizedPictureIsScaledRatherThanRefused() {
    ...
    var decoded = MuifliEditorFile.ToRawImage(MuifliEditorFile.FromRawImage(_Stripes(96, 72)));
  ...
  public void WhatIsEncodedSurvivesTheWriterAndTheReader() {
    var file = MuifliEditorFile.FromRawImage(_Stripes(160, 200));
    var restored = MuifliEditorReader.FromBytes(MuifliEditorWriter.ToBytes(file));

    Assert.That(_Rgb(MuifliEditorFile.ToRawImage(restored)), Is.EqualTo(_Rgb(MuifliEditorFile.ToRawImage(file))));
  }
}
