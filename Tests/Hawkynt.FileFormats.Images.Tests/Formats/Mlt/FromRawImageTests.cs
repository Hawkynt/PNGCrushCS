...
[TestFixture]
public sealed class MltFromRawImageTests {
  ...
  public void EncodeThenDecode_ReproducesAPictureTheFormatCanHold() {
    ...
    var decoded = MltFile.ToRawImage(MltFile.FromRawImage(source));
  ...
  public void ADifferentlySizedPictureIsScaledRatherThanRefused() {
    ...
    var decoded = MltFile.ToRawImage(MltFile.FromRawImage(_Stripes(96, 72)));
  ...
  public void FromRawImage_Null_Throws()
    => Assert.Throws<ArgumentNullException>(() => MltFile.FromRawImage(null!));

  [Test]
  [Category("Unit")]
  public void WhatIsEncodedSurvivesTheWriterAndTheReader() {
    var file = MltFile.FromRawImage(_Stripes(160, 200));
    var restored = MltReader.FromBytes(MltWriter.ToBytes(file));

    Assert.That(_Rgb(MltFile.ToRawImage(restored)), Is.EqualTo(_Rgb(MltFile.ToRawImage(file))));
  }
}
