...
public sealed class RoundTripTests {
  ...
  public void RoundTrip_AllFieldsPreserved() {
    ...
    var bytes = MltWriter.ToBytes(original);
    var restored = MltReader.FromBytes(bytes);
  ...
  public void RoundTrip_ViaFile_PreservesData() {
    ...
    var tmpPath = Path.Combine(Path.GetTempPath(), Guid.NewGuid() + ".mlt");
    try {
      var bytes = MltWriter.ToBytes(original);
      File.WriteAllBytes(tmpPath, bytes);
      var restored = MltReader.FromFile(new FileInfo(tmpPath));
  ...
  public void RoundTrip_ToRawImage_ProducesCorrectDimensions() {
    ...
    var raw = MltFile.ToRawImage(original);
