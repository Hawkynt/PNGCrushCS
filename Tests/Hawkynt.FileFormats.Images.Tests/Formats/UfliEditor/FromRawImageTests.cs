...
[TestFixture]
public sealed class UfliEditorFromRawImageTests {
  ...
  public void EncodeThenDecode_ReproducesAPictureTheFormatCanHold() {
    ...
    var decoded = UfliEditorFile.ToRawImage(UfliEditorFile.FromRawImage(source));
  ...
  public void ADifferentlySizedPictureIsScaledRatherThanRefused() {
    ...
    var decoded = UfliEditorFile.ToRawImage(UfliEditorFile.FromRawImage(_Stripes(96, 72)));
  ...
  public void WhatIsEncodedSurvivesTheWriterAndTheReader() {
    var file = UfliEditorFile.FromRawImage(_Stripes(160, 200));
    var restored = UfliEditorReader.FromBytes(UfliEditorWriter.ToBytes(file));

    Assert.That(_Rgb(UfliEditorFile.ToRawImage(restored)), Is.EqualTo(_Rgb(UfliEditorFile.ToRawImage(file))));
  }
}
