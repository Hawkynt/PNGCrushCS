...
[TestFixture]
public sealed class ChampionsInterlaceFromRawImageTests {
  ...
  /// </summary>
  /// <remarks>
  /// Two colours to a cell and to every raster line of one, which is inside what a multicolour interlace pair can hold,
  ...
  public void EncodeThenDecode_ReproducesAPictureTheFormatCanHold() {
    ...
    var decoded = ChampionsInterlaceFile.ToRawImage(ChampionsInterlaceFile.FromRawImage(source));
  ...
  public void ADifferentlySizedPictureIsScaledRatherThanRefused() {
    ...
    var decoded = ChampionsInterlaceFile.ToRawImage(ChampionsInterlaceFile.FromRawImage(_Stripes(96, 72)));
  ...
  public void FromRawImage_Null_Throws()
    => Assert.Throws<ArgumentNullException>(() => ChampionsInterlaceFile.FromRawImage(null!));

  [Test]
  [Category("Unit")]
  public void WhatIsEncodedSurvivesTheWriterAndTheReader() {
    var file = ChampionsInterlaceFile.FromRawImage(_Stripes(160, 200));
    var restored = ChampionsInterlaceReader.FromBytes(ChampionsInterlaceWriter.ToBytes(file));

    Assert.That(_Rgb(ChampionsInterlaceFile.ToRawImage(restored)), Is.EqualTo(_Rgb(ChampionsInterlaceFile.ToRawImage(file))));
  }
}
