...
[TestFixture]
public sealed class DrazPaintFromRawImageTests {
  ...
  public void EncodeThenDecode_ReproducesAPictureTheFormatCanHold() {
    ...
    var decoded = DrazPaintFile.ToRawImage(DrazPaintFile.FromRawImage(source));
  ...
  public void ADifferentlySizedPictureIsScaledRatherThanRefused() {
    ...
    var decoded = DrazPaintFile.ToRawImage(DrazPaintFile.FromRawImage(_Stripes(96, 72)));
  ...
  public void WhatIsEncodedSurvivesTheWriterAndTheReader() {
    var file = DrazPaintFile.FromRawImage(_Stripes(160, 200));
    var restored = DrazPaintReader.FromBytes(DrazPaintWriter.ToBytes(file));

    Assert.That(_Rgb(DrazPaintFile.ToRawImage(restored)), Is.EqualTo(_Rgb(DrazPaintFile.ToRawImage(file))));
  }
}
