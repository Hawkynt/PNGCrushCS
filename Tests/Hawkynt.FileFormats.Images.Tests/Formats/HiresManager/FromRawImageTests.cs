...
[TestFixture]
public sealed class HiresManagerFromRawImageTests {
  ...
  public void EncodeThenDecode_ReproducesAPictureTheFormatCanHold() {
    var source = _Stripes(296, 192);
    var decoded = HiresManagerFile.ToRawImage(HiresManagerFile.FromRawImage(source));
  ...
  public void ADifferentlySizedPictureIsScaledRatherThanRefused() {
    ...
    var decoded = HiresManagerFile.ToRawImage(HiresManagerFile.FromRawImage(_Stripes(96, 72)));
  ...
  public void WhatIsEncodedSurvivesTheWriterAndTheReader() {
    var file = HiresManagerFile.FromRawImage(_Stripes(296, 192));
    var restored = HiresManagerReader.FromBytes(HiresManagerWriter.ToBytes(file));

    Assert.That(_Rgb(HiresManagerFile.ToRawImage(restored)), Is.EqualTo(_Rgb(HiresManagerFile.ToRawImage(file))));
  }
}
