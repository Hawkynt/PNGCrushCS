...
[TestFixture]
public sealed class AfliFromRawImageTests {
  ...
  public void EncodeThenDecode_ReproducesAPictureTheFormatCanHold() {
    ...
    var decoded = AfliFile.ToRawImage(AfliFile.FromRawImage(source));
  ...
  public void ADifferentlySizedPictureIsScaledRatherThanRefused() {
    ...
    var decoded = AfliFile.ToRawImage(AfliFile.FromRawImage(_Stripes(96, 72)));
  ...
  public void FromRawImage_Null_Throws()
    => Assert.Throws<ArgumentNullException>(() => AfliFile.FromRawImage(null!));

  [Test]
  [Category("Unit")]
  public void WhatIsEncodedSurvivesTheWriterAndTheReader() {
    var file = AfliFile.FromRawImage(_Stripes(296, 200));
    var restored = AfliReader.FromBytes(AfliWriter.ToBytes(file));

    Assert.That(_Rgb(AfliFile.ToRawImage(restored)), Is.EqualTo(_Rgb(AfliFile.ToRawImage(file))));
  }
}
