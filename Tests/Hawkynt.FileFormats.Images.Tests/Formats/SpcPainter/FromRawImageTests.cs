...
[TestFixture]
public sealed class SpcPainterFileFromRawImageTests {
  ...
  public void EncodeThenDecode_ReproducesAPictureTheFormatCanHold() {
    ...
    var decoded = SpcPainterFile.ToRawImage(SpcPainterFile.FromRawImage(source));
  ...
  public void ADifferentlySizedPictureIsScaledRatherThanRefused() {
    ...
    var decoded = SpcPainterFile.ToRawImage(SpcPainterFile.FromRawImage(_Source(96, 72)));
  ...
  public void FromRawImage_Null_Throws()
    => Assert.Throws<ArgumentNullException>(() => SpcPainterFile.FromRawImage(null!));

  [Test]
  [Category("Unit")]
  public void WhatIsEncodedSurvivesTheWriterAndTheReader() {
    var file = SpcPainterFile.FromRawImage(_Source(320, 200));
    var restored = SpcPainterReader.FromBytes(SpcPainterWriter.ToBytes(file));

    Assert.That(_Rgb(SpcPainterFile.ToRawImage(restored)), Is.EqualTo(_Rgb(SpcPainterFile.ToRawImage(file))));
  }
}
