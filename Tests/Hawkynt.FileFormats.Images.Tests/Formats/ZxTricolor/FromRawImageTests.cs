...
public sealed class FromRawImageTests {
  ...
  public void RoundTrip_CheckerboardOfCells_ReproducesExactly() {
    ...
    var file = ZxTricolorFile.FromRawImage(source);
    var restored = ZxTricolorReader.FromBytes(ZxTricolorWriter.ToBytes(file));
    var decoded = ZxTricolorFile.ToRawImage(restored);
  ...
  public void FromRawImage_ScalesAPictureOfAnyOtherSize() {
    ...
    var small = ZxTricolorFile.ToRawImage(ZxTricolorFile.FromRawImage(Raw(100, 100)));
    var large = ZxTricolorFile.ToRawImage(ZxTricolorFile.FromRawImage(Raw(640, 480)));
  ...
  [Test]
  [Category("Unit")]
  public void FromRawImage_EncodesAllThreeScreensIdentically() {
    var file = ZxTricolorFile.FromRawImage(_Checkerboard());

    Assert.That(file.BitmapData2, Is.EqualTo(file.BitmapData1));
    Assert.That(file.BitmapData3, Is.EqualTo(file.BitmapData1));
    Assert.That(file.AttributeData2, Is.EqualTo(file.AttributeData1));
    Assert.That(file.AttributeData3, Is.EqualTo(file.AttributeData1));
  }
}
