...
public sealed class FromRawImageTests {
  ...
  public void RoundTrip_CheckerboardOfCells_ReproducesExactly() {
    ...
    var file = ZxFlashFile.FromRawImage(source);
    var restored = ZxFlashReader.FromBytes(ZxFlashWriter.ToBytes(file));
    var decoded = ZxFlashFile.ToRawImage(restored);
  ...
  public void FromRawImage_ScalesAPictureOfAnyOtherSize() {
    ...
    var small = ZxFlashFile.ToRawImage(ZxFlashFile.FromRawImage(Raw(100, 100)));
    var large = ZxFlashFile.ToRawImage(ZxFlashFile.FromRawImage(Raw(640, 480)));
  ...
  [Test]
  [Category("Unit")]
  public void FromRawImage_ReportsASingleFrame()
    => Assert.That(ZxFlashFile.FromRawImage(_Checkerboard()).FrameCount, Is.EqualTo(1));
}
