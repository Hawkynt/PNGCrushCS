...
[TestFixture]
public sealed class ArtDirectorFileFromRawImageTests {
  ...
  public void EncodeThenDecode_ReproducesAPictureTheFormatCanHold() {
    ...
    var decoded = ArtDirectorFile.ToRawImage(ArtDirectorFile.FromRawImage(source));
  ...
  public void ADifferentlySizedPictureIsScaledRatherThanRefused() {
    ...
    var decoded = ArtDirectorFile.ToRawImage(ArtDirectorFile.FromRawImage(_Source(96, 72)));
  ...
  public void FromRawImage_Null_Throws()
    => Assert.Throws<ArgumentNullException>(() => ArtDirectorFile.FromRawImage(null!));

  [Test]
  [Category("Unit")]
  public void WhatIsEncodedSurvivesTheWriterAndTheReader() {
    var file = ArtDirectorFile.FromRawImage(_Source(320, 200));
    var restored = ArtDirectorReader.FromBytes(ArtDirectorWriter.ToBytes(file));

    Assert.That(_Rgb(ArtDirectorFile.ToRawImage(restored)), Is.EqualTo(_Rgb(ArtDirectorFile.ToRawImage(file))));
  }
}
