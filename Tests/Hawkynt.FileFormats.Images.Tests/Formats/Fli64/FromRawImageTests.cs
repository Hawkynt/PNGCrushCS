...
public sealed class FromRawImageTests {
  ...
  /// inside FLI's per-cell/per-strip colour budget, so quantization and selection are both exact.</summary>
  private static RawImage _SolidCells() {
    const int width = Fli64File.FixedWidth, height = Fli64File.FixedHeight;
  ...
  public void RoundTrip_SolidCells_ReproducesExactly() {
    ...
    var file = Fli64File.FromRawImage(source);
    var restored = Fli64Reader.FromBytes(Fli64Writer.ToBytes(file));
    var decoded = Fli64File.ToRawImage(restored);
  ...
  public void FromRawImage_ScalesAPictureOfAnyOtherSize() {
    ...
    var small = Fli64File.ToRawImage(Fli64File.FromRawImage(Raw(100, 100)));
    var large = Fli64File.ToRawImage(Fli64File.FromRawImage(Raw(640, 480)));
  ...
  public void FromRawImage_ProducesTheStandardSectionSizes() {
    var file = Fli64File.FromRawImage(_SolidCells());

    Assert.That(file.BitmapData.Length, Is.EqualTo(8000));
    Assert.That(file.ScreenData.Length, Is.EqualTo(8000));
