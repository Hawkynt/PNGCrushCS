...
[TestFixture]
public sealed class DoodleCompFromRawImageTests {
  ...
  public void EncodeThenDecode_ReproducesAPictureTheFormatCanHold() {
    ...
    var decoded = DoodleCompFile.ToRawImage(DoodleCompFile.FromRawImage(source));
  ...
  public void ADifferentlySizedPictureIsScaledRatherThanRefused() {
    ...
    var decoded = DoodleCompFile.ToRawImage(DoodleCompFile.FromRawImage(_Stripes(96, 72)));
  ...
  public void FromRawImage_Null_Throws()
    => Assert.Throws<ArgumentNullException>(() => DoodleCompFile.FromRawImage(null!));

  [Test]
  [Category("Unit")]
  public void WhatIsEncodedSurvivesTheWriterAndTheReader() {
    var file = DoodleCompFile.FromRawImage(_Stripes(320, 200));
    var restored = DoodleCompReader.FromBytes(DoodleCompWriter.ToBytes(file));

    Assert.That(_Rgb(DoodleCompFile.ToRawImage(restored)), Is.EqualTo(_Rgb(DoodleCompFile.ToRawImage(file))));
  }
}
