...
[TestFixture]
public sealed class HardInterlaceFromRawImageTests {
  ...
  public void EncodeThenDecode_ReproducesAPictureTheFormatCanHold() {
    ...
    var decoded = HardInterlaceFile.ToRawImage(HardInterlaceFile.FromRawImage(source));
  ...
  public void ADifferentlySizedPictureIsScaledRatherThanRefused() {
    ...
    var decoded = HardInterlaceFile.ToRawImage(HardInterlaceFile.FromRawImage(_Stripes(96, 72)));
  ...
  public void WhatIsEncodedSurvivesTheWriterAndTheReader() {
    var file = HardInterlaceFile.FromRawImage(_Stripes(320, 200));
    var restored = HardInterlaceReader.FromBytes(HardInterlaceWriter.ToBytes(file));

    Assert.That(_Rgb(HardInterlaceFile.ToRawImage(restored)), Is.EqualTo(_Rgb(HardInterlaceFile.ToRawImage(file))));
  }
}
