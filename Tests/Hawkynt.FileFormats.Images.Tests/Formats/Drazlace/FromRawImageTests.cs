...
[TestFixture]
public sealed class DrazlaceFromRawImageTests {
  ...
  public void EncodeThenDecode_ReproducesAPictureTheFormatCanHold() {
    ...
    var decoded = DrazlaceFile.ToRawImage(DrazlaceFile.FromRawImage(source));
  ...
  public void ADifferentlySizedPictureIsScaledRatherThanRefused() {
    ...
    var decoded = DrazlaceFile.ToRawImage(DrazlaceFile.FromRawImage(_Stripes(96, 72)));
  ...
  public void WhatIsEncodedSurvivesTheWriterAndTheReader() {
    var file = DrazlaceFile.FromRawImage(_Stripes(160, 200));
    var restored = DrazlaceReader.FromBytes(DrazlaceWriter.ToBytes(file));

    Assert.That(_Rgb(DrazlaceFile.ToRawImage(restored)), Is.EqualTo(_Rgb(DrazlaceFile.ToRawImage(file))));
  }
}
