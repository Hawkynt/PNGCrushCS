...
[TestFixture]
public sealed class DoodlePackedFromRawImageTests {
  ...
  /// </summary>
  /// <remarks>
  /// Two colours to a cell and to every raster line of one, which is inside what a Doodle screen can hold,
  ...
  public void EncodeThenDecode_ReproducesAPictureTheFormatCanHold() {
    ...
    var decoded = DoodlePackedFile.ToRawImage(DoodlePackedFile.FromRawImage(source));
  ...
  public void ADifferentlySizedPictureIsScaledRatherThanRefused() {
    ...
    var decoded = DoodlePackedFile.ToRawImage(DoodlePackedFile.FromRawImage(_Stripes(96, 72)));
  ...
  public void FromRawImage_Null_Throws()
    => Assert.Throws<ArgumentNullException>(() => DoodlePackedFile.FromRawImage(null!));

  [Test]
  [Category("Unit")]
  public void WhatIsEncodedSurvivesTheWriterAndTheReader() {
    var file = DoodlePackedFile.FromRawImage(_Stripes(320, 200));
    var restored = DoodlePackedReader.FromBytes(DoodlePackedWriter.ToBytes(file));

    Assert.That(_Rgb(DoodlePackedFile.ToRawImage(restored)), Is.EqualTo(_Rgb(DoodlePackedFile.ToRawImage(file))));
  }
}
