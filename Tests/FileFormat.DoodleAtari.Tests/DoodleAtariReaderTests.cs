...
[TestFixture]
public sealed class DoodleAtariReaderTests {
  ...
  public void FromBytes_Null_ThrowsArgumentNullException() {
    Assert.Throws<ArgumentNullException>(() => DoodleAtariReader.FromBytes(null!));
  }

  [Test]
  [Category("Unit")]
  public void FromFile_Null_ThrowsArgumentNullException() {
    Assert.Throws<ArgumentNullException>(() => DoodleAtariReader.FromFile(null!));
  }

  [Test]
  [Category("Unit")]
  public void FromFile_Missing_ThrowsFileNotFoundException() {
    var missing = new FileInfo(Path.Combine(Path.GetTempPath(), Guid.NewGuid() + ".doo"));
    Assert.Throws<FileNotFoundException>(() => DoodleAtariReader.FromFile(missing));
  }

  [Test]
  [Category("Unit")]
  public void FromStream_Null_ThrowsArgumentNullException() {
    Assert.Throws<ArgumentNullException>(() => DoodleAtariReader.FromStream(null!));
  }
  ...
  public void FromBytes_TooSmall_ThrowsInvalidDataException() {
    ...
    Assert.Throws<InvalidDataException>(() => DoodleAtariReader.FromBytes(tooSmall));
  }
  ...
  public void FromBytes_TooLarge_ThrowsInvalidDataException() {
    ...
    Assert.Throws<InvalidDataException>(() => DoodleAtariReader.FromBytes(tooLarge));
  }
  ...
  public void FromBytes_ExactSize_Parses() {
    ...
    var result = DoodleAtariReader.FromBytes(data);
  ...
  public void FromStream_Valid() {
    ...
    var result = DoodleAtariReader.FromStream(ms);
  ...
  public void ToRawImage_AllZeros_ProducesWhiteImage() {
    ...
    var raw = DoodleAtariFile.ToRawImage(file);
  ...
  public void RoundTrip_PixelDataPreserved() {
    ...
    var original = new DoodleAtariFile {
      PixelData = data
    };

    var bytes = DoodleAtariWriter.ToBytes(original);
    var restored = DoodleAtariReader.FromBytes(bytes);
  ...
  public void RoundTrip_ViaFile() {
    ...
    var tempPath = Path.Combine(Path.GetTempPath(), Guid.NewGuid() + ".doo");
    try {
      var bytes = DoodleAtariWriter.ToBytes(original);
      File.WriteAllBytes(tempPath, bytes);

      var restored = DoodleAtariReader.FromFile(new FileInfo(tempPath));
  ...
  public void WriterToBytes_Produces32000Bytes() {
    ...
    var bytes = DoodleAtariWriter.ToBytes(file);
  ...
  public void DataType_ExpectedFileSize_Is32000() {
    Assert.That(DoodleAtariFile.ExpectedFileSize, Is.EqualTo(32000));
  }
}
