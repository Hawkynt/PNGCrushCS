...
using System.IO;
using System.Linq;
using FileFormat.Codecs.Mpeg1.Tests;
using FileFormat.Core;
using Hawkynt.FileFormats.Video;

namespace FileFormat.Mpeg1Video.Tests;
...
/// </remarks>
[TestFixture]
public sealed class Mpeg1VideoReaderTests {

  [Test]
  [Category("Unit")]
  public void FromBytes_Null_ThrowsArgumentNullException()
    => Assert.Throws<ArgumentNullException>(() => Mpeg1VideoReader.FromBytes(null!));
  ...
  public void DataThatDoesNotOpenWithASequenceHeaderIsRefused() {
    ...
    var failure = Assert.Throws<InvalidDataException>(
      () => Mpeg1VideoReader.FromBytes([0x00, 0x00, 0x01, 0x00, 0x00, 0x00]));
  ...
  public void AGroupHeaderTravelsWithThePictureAfterIt() {
    var stream = new Mpeg1TestStream().SequenceHeader(16, 16).GroupOfPictures();
    _Picture(stream, 1);
  ...
  public void AStreamWithNoPictureAtAllYieldsNoPackets()
    => Assert.That(_Packets(new Mpeg1TestStream().SequenceHeader(16, 16).GroupOfPictures().End()), Is.Empty);

  [Test]
  [Category("Unit")]
  public void TheOneStreamIsVideoAndNamesTheCodecWithoutReadingAnyHeader() {
    var streams = Mpeg1VideoContainer.Streams(Mpeg1VideoReader.FromBytes(_Stream(pictures: 1)));
  ...
  public void AskingForAStreamTheFileDoesNotHaveWalksNothing()
    => Assert.That(Mpeg1VideoContainer.ReadPackets(Mpeg1VideoReader.FromBytes(_Stream(pictures: 2)), 1), Is.Empty);

  [Test]
  [Category("Unit")]
  public void TheContainerIsDetectedByItsSignature()
    => Assert.That(VideoFormatRegistry.Detect(_Stream(pictures: 1)), Is.EqualTo(VideoFormat.Mpeg1Video));

  [Test]
  [Category("Unit")]
  public void TheContainerAndTheCodecAreBothRegistered() {
    Assert.That(VideoFormatRegistry.AllFormats.Select(entry => entry.Format), Does.Contain(VideoFormat.Mpeg1Video));
  ...
  public void TheExtensionsTheContainerClaimsReachIt() {
    foreach (var extension in new[] { ".m1v", ".mpv", ".mpeg1video" })
      Assert.That(VideoFormatRegistry.ByExtension(extension), Does.Contain(VideoFormat.Mpeg1Video), extension);
  }
  ...
  private static void _Picture(Mpeg1TestStream stream, int codingType) {
  ...
  private static List<CodedPacket> _Packets(byte[] stream)
    => Mpeg1VideoContainer.ReadPackets(Mpeg1VideoReader.FromBytes(stream)).ToList();
