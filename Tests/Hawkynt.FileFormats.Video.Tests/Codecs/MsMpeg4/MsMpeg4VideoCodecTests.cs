...
public sealed class MsMpeg4VideoCodecTests {
  ...
  /// <summary>
  /// A Matroska file of the original DivX reaches the decoder under the name Matroska gives it.
  /// </summary>
  /// <remarks>
  /// The one of the three versions Matroska names at all, and the four-character code is absent there
  /// — a track states either a <c>CodecID</c> or a <c>BITMAPINFOHEADER</c> code, never both — so it
  /// has to be found by name or not at all.
  /// </remarks>
  [Test]
  [Category("Unit")]
  public void VersionThreeIsFoundUnderTheNameMatroskaGivesIt() {
    var stream = new MediaStreamInfo {
      Index = 0, Kind = MediaStreamKind.Video, CodecId = "V_MPEG4/MS/V3", Width = 64, Height = 64,
    };

    Assert.That(MsMpeg4VideoDecoder.Accepts(stream), Is.True);
    Assert.That((int)MsMpeg4VideoDecoder.Create(stream).Version, Is.EqualTo(3));
  }
