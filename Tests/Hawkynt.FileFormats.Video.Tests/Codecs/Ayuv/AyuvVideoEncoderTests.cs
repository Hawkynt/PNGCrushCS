...
/// <summary>
/// The ayuv encoder, against the package's own decoder and the packing written out by hand.
/// </summary>
[TestFixture]
public class AyuvVideoEncoderTests {
  ...
  public void DescribesAStreamTheDecoderAccepts() {
    var encoder = AyuvVideoEncoder.Create(_Stream(17, 9, 2));

    var described = encoder.DescribeStream();

    Assert.Multiple(() => {
      Assert.That(AyuvVideoEncoder.Codec, Is.EqualTo(_AYUV));
      Assert.That(described.Codec, Is.EqualTo(_AYUV));
      ...
      Assert.That(AyuvVideoDecoder.Accepts(described), Is.True);
  ...
  [Test]
  [Category("Unit")]
  public void PacksVThenUThenYThenOpaqueAlphaFromPlanarInput() {
    ...
    var encoder = AyuvVideoEncoder.Create(_Stream(2, 1));

    Assert.That(encoder.TryEncode(frame, 0, out var packet), Is.True);

    Assert.That(packet.Data.ToArray(), Is.EqualTo(new byte[] { 240, 90, 81, 255, 16, 128, 235, 255 }));
  }
  ...
  public void TakesRgb24WithOpaqueAlpha() {
    ...
    Assert.That(packet.Data.ToArray(), Is.EqualTo(new byte[] { 128, 128, 235, 255, 128, 128, 16, 255 }));
  }

  [Test]
  [Category("Unit")]
  public void PacketsAreKeyFramesCarryingTheTimestampsGiven() {
    var encoder = AyuvVideoEncoder.Create(_Stream(4, 2, 3));
  ...
  public void RefusesAGeometryChangeMidStream() {
    var encoder = AyuvVideoEncoder.Create(_Stream(4, 2));
  ...
  public void RefusesAStreamWithNoPixelsOrOfAnotherKind() {
    Assert.Multiple(() => {
      Assert.That(Assert.Throws<NotSupportedException>(() => AyuvVideoEncoder.Create(_Stream(4, 0)))!.Message, Does.Contain("4x0"));
      Assert.Throws<NotSupportedException>(() => AyuvVideoEncoder.Create(new() { Index = 0, Kind = MediaStreamKind.Audio, Width = 4, Height = 4 }));
    });
  }
}
