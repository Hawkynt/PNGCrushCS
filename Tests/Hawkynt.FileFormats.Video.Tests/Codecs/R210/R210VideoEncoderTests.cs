...
/// <summary>
/// The r210 encoder, against the package's own decoder and the word written out by hand.
/// </summary>
[TestFixture]
public class R210VideoEncoderTests {
  ...
  public void DescribesAStreamTheDecoderAccepts() {
    var encoder = R210VideoEncoder.Create(_Stream(33, 25, 1));

    var described = encoder.DescribeStream();

    Assert.Multiple(() => {
      Assert.That(R210VideoEncoder.Codec, Is.EqualTo(_R210));
      Assert.That(described.Codec, Is.EqualTo(_R210));
      ...
      Assert.That(R210VideoDecoder.Accepts(described), Is.True);
  ...
  [Test]
  [Category("Unit")]
  public void PacksRedHighGreenMiddleBlueLowBigEndianAndPadsTheRowToTwoHundredFiftySixBytes() {
    // R=500, G=300, B=700 in Rgb30's own layout is R | G<<10 | B<<20; r210's word is
    // R<<20 | G<<10 | B = 0x1F44B2BC, stored big-endian; alpha fully opaque in the source.
    ...
    var encoder = R210VideoEncoder.Create(_Stream(1, 1));

    Assert.That(encoder.TryEncode(frame, 0, out var packet), Is.True);

    var data = packet.Data.ToArray();
    Assert.Multiple(() => {
      Assert.That(data, Has.Length.EqualTo(256));
      Assert.That(data[..4], Is.EqualTo(new byte[] { 0x1F, 0x44, 0xB2, 0xBC }));
      Assert.That(data[4..], Is.All.Zero);
    });
  }

  [Test]
  [Category("Unit")]
  public void ARowOfSixtyFourPixelsNeedsNoPaddingAndOneOfSixtyFiveNeedsAWholeSecondBlock() {
    Assert.That(R210VideoEncoder.Create(_Stream(64, 2)).TryEncode(_RandomRgb30(64, 2, 1), 0, out var exact), Is.True);
    Assert.That(R210VideoEncoder.Create(_Stream(65, 2)).TryEncode(_RandomRgb30(65, 2, 1), 0, out var padded), Is.True);

    Assert.Multiple(() => {
      Assert.That(exact.Data.Length, Is.EqualTo(512));
      Assert.That(padded.Data.Length, Is.EqualTo(1024));
    });
  }
  ...
  public void RoundTripsPseudoRandomRgb30ThroughTheRegistryDecoder(int width, int height) {
    var encoder = R210VideoEncoder.Create(_Stream(width, height));
    var decoder = VideoFormatRegistry.CreateDecoder(encoder.DescribeStream());

    for (var i = 0; i < 3; ++i) {
      var source = _RandomRgb30(width, height, 4000 + i);
      Assert.That(encoder.TryEncode(source, i, out var packet), Is.True);
      Assert.That(packet.Data.Length, Is.EqualTo((width * 4 + 255) / 256 * 256 * height));
  ...
  public void DropsTheTwoAlphaBitsItHasNoRoomFor() {
    ...
    Assert.That(packet.Data.ToArray()[..4], Is.EqualTo(new byte[] { 0x1F, 0x44, 0xB2, 0xBC }));
  }

  [Test]
  [Category("Unit")]
  public void PacketsAreKeyFramesCarryingTheTimestampsGiven() {
    var encoder = R210VideoEncoder.Create(_Stream(4, 2, 3));
  ...
  public void RefusesAGeometryChangeMidStream() {
    var encoder = R210VideoEncoder.Create(_Stream(4, 2));
  ...
  public void RefusesAPixelFormatItCannotCarryLosslesslyByName() {
    var encoder = R210VideoEncoder.Create(_Stream(2, 1));
  ...
  public void RefusesAStreamWithNoPixelsOrOfAnotherKind() {
    Assert.Multiple(() => {
      Assert.That(Assert.Throws<NotSupportedException>(() => R210VideoEncoder.Create(_Stream(0, 4)))!.Message, Does.Contain("0x4"));
      Assert.Throws<NotSupportedException>(() => R210VideoEncoder.Create(new() { Index = 0, Kind = MediaStreamKind.Audio, Width = 4, Height = 4 }));
    });
  }
}
