...
public sealed class H264VideoDecoderTests {
  ...
  public void OneIntraMacroblockDecodesToTheMidGreyItsPredictionSpecifies() {
    ...
    Assert.That(frames[0].PixelData, Has.Length.EqualTo(16 * 16 * 3));
    Assert.That(frames[0].PixelData, Is.All.EqualTo(130));
  }
  ...
  public void ASkippedPredictedPictureRepeatsItsReference() {
    var stream = new H264TestStream()
      ...
      .BeginSliceHeader(frameNum: 1).Unsigned(1).EndNal(1, 2)
      .ToArray();
  ...
  public void AReferenceIndexTheListDoesNotHoldIsRefusedRatherThanGuessed() {
    var stream = new H264TestStream()
      ...
      .ToArray();
    Assert.That(() => _Decode(stream), Throws.TypeOf<InvalidDataException>().With.Message.Contains("reference index 1"));
  }
  ...
  public void AGapInTheReferenceFrameNumberingIsRefusedRatherThanDecodedThrough() {
    var stream = new H264TestStream()
      ...
      .ToArray();
    Assert.That(() => _Decode(stream), Throws.TypeOf<InvalidDataException>().With.Message.Contains("frame_num jumps from 0 to 3"));
  }
  ...
  public void FlexibleMacroblockOrderingIsRefusedByName() {
    var stream = new H264TestStream()
      ...
      .BeginIdrSliceHeader().FlatIntra16x16Macroblock().EndNal(5, 3)
      .ToArray();
    Assert.That(() => _Decode(stream), Throws.TypeOf<NotSupportedException>().With.Message.Contains("slice groups"));
  }

  [Test]
  public void WeightedPredictionFlagIsAcceptedForPicturesThatDoNotUseInterPrediction() {
    var stream = new H264TestStream()
      .SequenceParameterSet()
  ...
  public void InterlacedCodingIsRefusedByName() {
    var stream = new H264TestStream()
      ...
      .BeginIdrSliceHeader().FlatIntra16x16Macroblock().EndNal(5, 3)
      .ToArray();
    Assert.That(() => _Decode(stream), Throws.TypeOf<NotSupportedException>().With.Message.Contains("frame_mbs_only_flag"));
  }
  ...
  public void SliceDataPartitioningIsRefusedByName() {
    var stream = new H264TestStream()
      .SequenceParameterSet()
      .PictureParameterSet()
      .BeginIdrSliceHeader().FlatIntra16x16Macroblock().EndNal(2, 3)
      .ToArray();
    Assert.That(() => _Decode(stream), Throws.TypeOf<NotSupportedException>().With.Message.Contains("slice data partitioning"));
  }
  ...
  public void ASliceNamingAParameterSetTheStreamNeverSentIsRefused() {
    var stream = new H264TestStream()
      ...
      .BeginIdrSliceHeader().FlatIntra16x16Macroblock().EndNal(5, 3)
      .ToArray();
    Assert.That(() => _Decode(stream), Throws.TypeOf<InvalidDataException>().With.Message.Contains("picture parameter set"));
  }
  ...
  public void APictureWhoseSlicesLeaveMacroblocksUncodedIsRefused() {
    var stream = new H264TestStream()
      ...
      .BeginIdrSliceHeader().FlatIntra16x16Macroblock().EndNal(5, 3)
      .ToArray();
    Assert.That(() => _Decode(stream), Throws.TypeOf<InvalidDataException>().With.Message.Contains("covered by no slice"));
  }
  ...
  public void APredictedSliceWithNoReferenceInTheBufferIsRefused() {
    var stream = new H264TestStream()
      ...
      .BeginSliceHeader(frameNum: 3).Unsigned(1).EndNal(1, 2)
      .ToArray();
    Assert.That(() => _Decode(stream), Throws.TypeOf<InvalidDataException>().With.Message.Contains("no reference picture"));
  }

  [Test]
  public void ANalUnitWithItsForbiddenBitSetIsRefused() {
    byte[] stream = [0x00, 0x00, 0x00, 0x01, 0x87, 0x42, 0x00, 0x0A, 0x80];
    Assert.That(() => _DecodeWholeStreamAsOnePacket(stream), Throws.TypeOf<InvalidDataException>().With.Message.Contains("forbidden_zero_bit"));
  }
