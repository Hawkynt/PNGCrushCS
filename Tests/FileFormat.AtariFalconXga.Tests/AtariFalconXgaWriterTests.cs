...
[TestFixture]
public sealed class AtariFalconXgaWriterTests {
  ...
  public void ToBytes_StartsWithDimensionsBigEndian() {
    ...
    var bytes = AtariFalconXgaWriter.ToBytes(file);
  ...
  public void ToBytes_FileSizeCorrect() {
    ...
    Assert.That(bytes.Length, Is.EqualTo(AtariFalconXgaHeader.StructSize + 16 * 8 * 2));
  }
  ...
  public void ToBytes_PixelDataPreserved() {
    ...
    Assert.That(bytes[AtariFalconXgaHeader.StructSize], Is.EqualTo(0xF8));
    Assert.That(bytes[AtariFalconXgaHeader.StructSize + 1], Is.EqualTo(0x00));
  ...
  public void ToBytes_ShortPixelData_PadsWithZeros() {
    ...
    Assert.That(bytes.Length, Is.EqualTo(AtariFalconXgaHeader.StructSize + 4 * 2 * 2));
    Assert.That(bytes[AtariFalconXgaHeader.StructSize + 4], Is.EqualTo(0));
  }
}
