...
[TestFixture]
public sealed class ExtendedGemImgWriterTests {

  [Test]
  [Category("Unit")]
  public void ToBytes_Null_ThrowsArgumentNullException() {
    Assert.Throws<ArgumentNullException>(() => ExtendedGemImgWriter.ToBytes(null!));
  }

  [Test]
  [Category("Unit")]
  public void ToBytes_StartsWithValidHeader() {
    var file = new ExtendedGemImgFile {
      Version = 1,
      Width = 16,
      Height = 2,
      NumPlanes = 1,
      ...
      PaletteData = _BuildPalette(2),
      PixelData = new byte[2 * 2]
    };

    var bytes = ExtendedGemImgWriter.ToBytes(file);
  ...
  [Test]
  [Category("Unit")]
  public void ToBytes_ContainsXimgMarker() {
    var file = new ExtendedGemImgFile {
      Version = 1,
      Width = 8,
      Height = 1,
    ...
    // XIMG marker at offset 16 (after standard header)
    var marker1 = BinaryPrimitives.ReadInt16BigEndian(bytes.AsSpan(16));
    var marker2 = BinaryPrimitives.ReadInt16BigEndian(bytes.AsSpan(18));
    Assert.That(marker1, Is.EqualTo(0x5849)); // "XI"
    Assert.That(marker2, Is.EqualTo(0x4D47)); // "MG"
  }

  [Test]
  [Category("Unit")]
  public void ToBytes_ColorModelIsWritten() {
    var file = new ExtendedGemImgFile {
      Version = 1,
      Width = 8,
      Height = 1,
    ...
    var colorModel = BinaryPrimitives.ReadInt16BigEndian(bytes.AsSpan(20));
    Assert.That(colorModel, Is.EqualTo(1)); // CMY
  }

  [Test]
  [Category("Unit")]
  public void ToBytes_PaletteDataIsWritten() {
    var paletteData = new short[] { 1000, 0, 0, 0, 1000, 0 }; // Red, Green
    var file = new ExtendedGemImgFile {
      Version = 1,
      Width = 8,
      Height = 1,
    ...
    // Palette starts at offset 22 (16 header + 4 marker + 2 color model)
    var r0 = BinaryPrimitives.ReadInt16BigEndian(bytes.AsSpan(22));
    var g0 = BinaryPrimitives.ReadInt16BigEndian(bytes.AsSpan(24));
    var b0 = BinaryPrimitives.ReadInt16BigEndian(bytes.AsSpan(26));
    Assert.Multiple(() => {
      Assert.That(r0, Is.EqualTo(1000));
  ...
  private static short[] _BuildPalette(int count) {
    var palette = new short[count * 3];
    for (var i = 0; i < count; ++i) {
      var val = (short)(i * 1000 / Math.Max(count - 1, 1));
      palette[i * 3] = val;
      palette[i * 3 + 1] = val;
      palette[i * 3 + 2] = val;
    }
...
public sealed class DataTypeTests {
  ...
  [Test]
  [Category("Unit")]
  public void ExtendedGemImgColorModel_HasExpectedValues() {
    Assert.That((int)ExtendedGemImgColorModel.Rgb, Is.EqualTo(0));
    Assert.That((int)ExtendedGemImgColorModel.Cmy, Is.EqualTo(1));
    Assert.That((int)ExtendedGemImgColorModel.Pantone, Is.EqualTo(2));

    var values = Enum.GetValues<ExtendedGemImgColorModel>();
    Assert.That(values, Has.Length.EqualTo(3));
  }

  [Test]
  [Category("Unit")]
  public void ExtendedGemImgFile_Defaults() {
    var file = new ExtendedGemImgFile();
    Assert.Multiple(() => {
      Assert.That(file.Version, Is.EqualTo(0));
      Assert.That(file.Width, Is.EqualTo(0));
      Assert.That(file.Height, Is.EqualTo(0));
      Assert.That(file.NumPlanes, Is.EqualTo(0));
      Assert.That(file.ColorModel, Is.EqualTo(ExtendedGemImgColorModel.Rgb));
  ...
  public void ToRawImage_Null_ThrowsArgumentNullException() {
    Assert.Throws<ArgumentNullException>(() => ExtendedGemImgFile.ToRawImage(null!));
  }

  [Test]
  [Category("Unit")]
  public void FromRawImage_Null_ThrowsArgumentNullException() {
    Assert.Throws<ArgumentNullException>(() => ExtendedGemImgFile.FromRawImage(null!));
  }
  ...
  public void FromRawImage_WrongFormat_ThrowsArgumentException() {
    var raw = new RawImage {
      ...
      PixelData = new byte[16 * 2 * 3],
    };
    Assert.Throws<ArgumentException>(() => ExtendedGemImgFile.FromRawImage(raw));
  }
}
