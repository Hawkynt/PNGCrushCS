...
public sealed class RoundTripTests {
  ...
  public void RoundTrip_LowRes_PreservesAllFields() {
    ...
    var bytes = CrackArtWriter.ToBytes(original);
    var restored = CrackArtReader.FromBytes(bytes);
  ...
  public void RoundTrip_MediumRes_PreservesAllFields() {
    ...
    var original = new CrackArtFile {
      Width = 640,
      Height = 200,
      Resolution = CrackArtResolution.Medium,
      Palette = palette,
      PixelData = pixelData
    };
    ...
    Assert.Multiple(() => {
      ...
      Assert.That(restored.Resolution, Is.EqualTo(CrackArtResolution.Medium));
      Assert.That(restored.Palette, Is.EqualTo(original.Palette));
      Assert.That(restored.PixelData, Is.EqualTo(original.PixelData));
    });
  }
  ...
  public void RoundTrip_HighRes_PreservesAllFields() {
    ...
    Assert.Multiple(() => {
      ...
      Assert.That(restored.Resolution, Is.EqualTo(CrackArtResolution.High));
      Assert.That(restored.Palette, Is.EqualTo(original.Palette));
      Assert.That(restored.PixelData, Is.EqualTo(original.PixelData));
    });
  }
