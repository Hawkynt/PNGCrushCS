...
public sealed class RoundTripTests {
  ...
  public void RoundTrip_PreservesAllFields() {
    ...
    var original = new QuantumPaintFile {
      Palette = palette,
      PixelData = pixelData,
    };

    var bytes = QuantumPaintWriter.ToBytes(original);
    var restored = QuantumPaintReader.FromBytes(bytes);
  ...
  public void RoundTrip_ViaFile() {
    ...
    var tempPath = Path.Combine(Path.GetTempPath(), Guid.NewGuid() + ".pbx");
    try {
      File.WriteAllBytes(tempPath, QuantumPaintWriter.ToBytes(original));
      var restored = QuantumPaintReader.FromFile(new FileInfo(tempPath));
  ...
  public void RoundTrip_ViaRawImage() {
    ...
    var raw = QuantumPaintFile.ToRawImage(original);
    var restored = QuantumPaintFile.FromRawImage(raw);
    var rawBack = QuantumPaintFile.ToRawImage(restored);
