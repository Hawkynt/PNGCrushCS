...
public sealed class DataTypeTests {
  ...
  public void RawData_InitProperty() {
    ...
    var file = new AtariGfbFile { RawData = rawData };
  ...
  public void FileSize_Is7680() {
    Assert.That(AtariGfbFile.FileSize, Is.EqualTo(7680));
  }

  [Test]
  [Category("Unit")]
  public void BytesPerRow_Is40() {
    Assert.That(AtariGfbFile.BytesPerRow, Is.EqualTo(40));
  }

  [Test]
  [Category("Unit")]
  public void PixelWidth_Is320() {
    Assert.That(AtariGfbFile.PixelWidth, Is.EqualTo(320));
  }

  [Test]
  [Category("Unit")]
  public void PixelHeight_Is192() {
    Assert.That(AtariGfbFile.PixelHeight, Is.EqualTo(192));
  }
  ...
  [Test]
  [Category("Unit")]
  public void FileExtensions_ContainsGfb() {
    var exts = _GetStaticProperty<string[]>("FileExtensions");
    Assert.That(exts, Has.Length.EqualTo(1));
    Assert.That(exts, Does.Contain(".gfb"));
  }

  [Test]
  [Category("Unit")]
  public void PrimaryExtension_IsGfb() {
    var ext = _GetStaticProperty<string>("PrimaryExtension");
    Assert.That(ext, Is.EqualTo(".gfb"));
  }

  [Test]
  [Category("Unit")]
  public void ToRawImage_Null_Throws() {
    Assert.Throws<ArgumentNullException>(() => AtariGfbFile.ToRawImage(null!));
  }

  [Test]
  [Category("Unit")]
  public void ToRawImage_Format_IsIndexed1() {
    var file = new AtariGfbFile { RawData = new byte[7680] };
    var raw = AtariGfbFile.ToRawImage(file);
  ...
  public void FromRawImage_Null_Throws() {
    Assert.Throws<ArgumentNullException>(() => AtariGfbFile.FromRawImage(null!));
  }
  ...
  public void FromRawImage_WrongFormat_Throws() {
    ...
    Assert.Throws<ArgumentException>(() => AtariGfbFile.FromRawImage(raw));
  }
  ...
  private static T _GetStaticProperty<T>(string name) {
    var map = typeof(AtariGfbFile).GetInterfaceMap(typeof(IImageFileFormat<AtariGfbFile>));
