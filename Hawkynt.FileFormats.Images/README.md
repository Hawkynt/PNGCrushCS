...
Where it stands, against `djxl`, counted in pixels that differ out of the picture's own: a flat 64x64 two of 4,096; a single dot four and a single block six of the same; a sixteen-by-sixteen four of 256; a 96x96 143 of 9,216; a 100x100 with an alpha plane 159 of 10,000; a 128x96 162 of 12,288; a 128x128 185 of 16,384; a 64x64 gradient 116 and the same in colour 158 and in plasma 212 of 4,096; a 160x160 315 of 25,600; a 192x192 425 of 36,864; a 200x150 776 of 30,000; a 224x224 800 of 50,176; a 250x250 942 of 62,500 and a 256x256 947 of 65,536. Every one of those was a decoder that lost the bitstream before any of this, and four of them were refused as recently as the paragraph above. Close is still not what this package hands back, so all of them are still refused to callers.

What is left is a shorter list than it was. A frame in more than one group still parts company with its coefficient stream — usually in the first group, once in the second, and which of the two depends on the picture rather than on its size, so there is a systematic error left that sometimes cancels and sometimes does not. The scan order a transform states for itself is read correctly — the permutation stream returns its arithmetic decoder to the initial state, which is the format's own check that it was read exactly as written — but it is then discarded, and the coefficients are placed by the natural order instead; every picture whose blocks are large enough to want a permutation is therefore built with its coefficients in the wrong places, which is the plainest remaining reason the numbers above are not zero. The edge-preserving filter is parsed and not run, for the reason given above. Which histogram set a group was coded with is stated in a field whose width is the log of how many sets the frame declares, and every file measured here — up to a 1,024x768 one in twelve groups — declares exactly one, so the field is zero bits wide and reading it could not be told apart from not reading it; it is left alone rather than written blind. A frame whose token stream did not return the arithmetic decoder to its initial state, or whose blocks did not each run to the end, is refused; those catch a reader that has lost the bitstream, though not on their own one that has followed it and rebuilt the picture wrongly, so they back up conformance rather than substituting for it. **JPEG XR** decodes real codestreams through a managed port of JXRLib's T.832 core and writes real `WMPHOTO`; what it refuses is narrower than the whole format — a component count outside one, three or four, and interleaved alpha, which needs a planar BCC2/BCC3 codestream instead. Gray8, RGB24 and RGBA32 are exposed; other WIC layouts may be turned down at the container adapter even where the core handles them. Camera RAW supports DNG lossless JPEG, Canon CR2, Nikon NEF, Sony ARW2; other manufacturer-specific compressions are future work.
