...
One thing was tried and not kept. Rectangular shapes are twice as wrong as square ones — in a 64x64 plasma the three 8x4 blocks average 51 levels out against 22 for the 61 plain 8x8 ones — and the obvious suspect was that their quantisation tables are built with their two dimensions the wrong way round. Turning them round changes almost every picture by a percent or two, in both directions, and changes those three blocks not at all, so whatever is wrong with them is not that. It is left alone rather than landed on a number that moved.
