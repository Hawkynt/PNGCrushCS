...
- The JPEG XL pixel path is measured against libjxl on 104 files, not against every `.jxl` that exists; a few frame features, splines among them, are refused rather than decoded. Do not treat its internal round-trip as proof of arbitrary `.jxl` compatibility.
