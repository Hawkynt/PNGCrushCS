...
So every eight-bit count in the paragraphs above is too large, and the ones near the end are almost entirely pattern. Adding the same pattern to this decoder's output before comparing — like with like — the three fixtures kept for it go from 1,701, 4,889 and 9,218 samples differing to **0, 3 and 2**. Across the whole corpus of 49 lossy files, from 16x16 to 1,024x768, 290 samples of 6,174,252 differ, none by more than one level, and every one of them is a value sitting nearer a rounding boundary than the two decoders agree. Measured before either is rounded, the largest disagreement anywhere in the corpus is **0.011 of one eight-bit level**.
