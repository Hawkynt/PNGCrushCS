...
What that leaves is narrow and is not yet found: the shapes that divide the block are self-consistent, correctly fed, and correctly scaled at the two points that can be checked from the inside, and still land twenty levels out where a plain 8x8 lands under two. Finding it needs libjxl's own transforms to compare against coefficient by coefficient rather than another property that both agree on.
