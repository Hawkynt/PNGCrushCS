...
Which leaves the search where it deserves to be left: honestly open. Every part of the path has now been checked against something outside itself — the transforms against the definition, the orders against libjxl's own walk, the weights three separate ways, the filter against its source — and the shapes that divide the 8x8 block still land ten times further out than the ones that do not. Whatever is left is not any of the things that have been looked at.
