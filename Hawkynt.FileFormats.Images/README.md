...
With that, every one of the forty-five files measured here agrees with libjxl to within a single level at every sample, and to within about two ten-thousandths of a level before either is rounded. What is left is not a defect: the two decoders part only where a value sits closer to a rounding boundary than the difference between two independent float pipelines, which is where any decoder that is not a copy of libjxl's arithmetic will part from it.

The same precision settled two things the eight-bit measurement could not. The smoothing filter is doing real work and doing it right: without it the same picture was nine times further out. The edge-preserving one is very nearly a no-op on a photograph, because the encoder marks about half its blocks as not to be filtered — which is why connecting it measured neutral, and why that was not evidence against it. Close is still not what this package hands back, so all of them are still refused to callers.
