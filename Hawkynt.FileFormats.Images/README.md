...
Every lossy file in the corpus now decodes — 36 of them, from one group up to twelve — and every one of them is within about one percent of libjxl, most of them well within it. Counted in pixels that differ out of the picture's own: a flat 64x64 two of 4,096, a gradient two, a single dot two, the gradient in colour four, a single block three, a sixteen-by-sixteen two of 256 — those are within a level or two everywhere and are as near exact as this decoder gets. Then a 100x100 with an alpha plane 27 of 10,000; a 128x96 of a grey subject 28 of 12,288; a 64x64 plasma 11 of 4,096; a 200x150 plasma 128 of 30,000; a 300x200 gradient 114 of 60,000; a 96x96 97 of 9,216; a 128x128 149 of 16,384; a 160x160 210 of 25,600; a 192x192 269 of 36,864; a 224x224 356 of 50,176; a 250x250 403 of 62,500; a 256x256 419 of 65,536; a 300x200 419 of 60,000; a 400x300 in four groups 800 of 120,000; a 640x480 in six 1,961 of 307,200; and a 1,024x768 in twelve 3,996 of 786,432. The quantisation weights had the same fault as the transforms did and it was found by looking for it there: they are applied to the block as this decoder stores it, and they were being laid out the way the shape is named. No square shape can show that, and every rectangle was having its curve laid down the wrong axis — as was the corner shape, whose two curves are not each other's transpose. Building them the way round the block is takes a 128x96 of a grey subject from 28 differing pixels of 12,288 to 22, a 100x100 with an alpha plane from 27 of 10,000 to 16, a 64x64 plasma from 11.2 of 4,096 to 7.9, and does not move a single picture the other way.

One stage was still missing, and the flag for it had been sitting in the frame header parser all along with nothing reading it. A frame carries one value per block for its low frequencies, and quantising those leaves steps between neighbouring blocks in anything smooth; the encoder writes the frame expecting the decoder to take them back out, and says so by not asking to skip it. What keeps that safe is that the difference between a value and its neighbourhood is measured in whole quantisation steps rather than in levels, so a difference the quantiser could not have introduced is a real edge and is left alone, and the three planes decide together so a colour edge is not smoothed out of one while another keeps it. Running it takes a 300x200 gradient from 112 differing pixels of 60,000 to 59 — smooth pictures being exactly where those steps show — and the worst pixel of a single-block picture from 41 levels to 1. Nothing moves the other way. It is applied a group at a time, so on a frame in more than one group the blocks along a group's edge are smoothed against that edge rather than against their neighbours across it.

The worst single pixel anywhere is 74 levels and in most pictures it is under 30, where at the start of this work it was a picture that could not be read at all. Close is still not what this package hands back, so all of them are still refused to callers.
