...
Every lossy file in the corpus now decodes — 36 of them, from one group up to twelve. Against `djxl`, counted in pixels that differ out of the picture's own: a flat 64x64 and a 64x64 gradient two of 4,096, the gradient in colour four, a single dot four, a single block six, a sixteen-by-sixteen four of 256 — those are within one or two levels everywhere and are as near exact as this decoder gets. Then a 128x96 79 of 12,288; a 100x100 with an alpha plane 137 of 10,000; a 96x96 138 of 9,216; a 300x200 gradient in two groups 163 of 60,000; a 128x128 207 of 16,384; a 160x160 353 of 25,600; a 200x150 362 of 30,000; a 192x192 434 of 36,864; a 224x224 459 of 50,176; a 200x256 465 of 51,200; a 300x200 570 of 60,000; a 256x256 600 of 65,536; a 257x256 610 of 65,792; a 250x250 616 of 62,500; a 400x300 in four groups 1,102 of 120,000; a 640x480 in six 2,749 of 307,200; and a 1,024x768 in twelve 5,247 of 786,432 — under two percent everywhere, under one for everything larger than a thumbnail, and under a tenth of one percent at the top of that list. Every one of these was a decoder that lost the bitstream before any of this began. Close is still not what this package hands back, so all of them are still refused to callers.
...
Measuring per shape rather than per picture then said where the rest of the error is, and it is in one place. Taking a 128x96 apart by the shape of each block: its 151 plain 8x8 blocks are 2.0 levels out on average — as near exact as the flat pictures — while every shape that is not a plain 8x8 is between five and twenty-nine. The 8x8 path is right and the others are not.

Three things were tried against that and none of them is it, which is worth writing down so they are not tried again. The tables of a shape and its transpose are built here as two tables rather than the one the format gives them, laid out along opposite axes; building them as one, the shorter side first, moves a 8x16 from 29.0 to 26.5 and a 16x32 the other way from 22.1 to 25.0, and leaves a 16x8 and a 32x16 — which already had that layout — untouched at 5.5 and 6.8. So the same table serves a shape at 6.8 and its transpose at 25.0, and the table is therefore not what separates them. The three shapes that divide the block into smaller transforms have tables too short to cover it and fall back to the 8x8 one stretched; giving them the real thing — the smaller curve spread back over the block, which is what the format says — moves a 4x8 from 21.4 to 22.8 and an 8x4 from 21.0 to 21.8, the wrong way. And turning the rectangles' dimensions round wholesale moves almost every picture by a percent or two in both directions while moving the rectangular blocks themselves not at all.

Three corrections to the weights, each faithful to the format, each making those blocks no better or slightly worse. That is what it looks like when the weights are not the thing that is wrong: what is left is the inverse transforms of the shapes that are not a plain 8x8 — the pair that split the block in half, the four that quarter it, the 2x2 cascade, the Hornuss and the AFV corner, and the scaling of the rectangular transforms generally. None of the three was landed, because a faithful table on top of a transform that is wrong is not something this package can show is an improvement.
