...
The second was not a gap in a codec but a wrong idea about what a file is. Two fixtures state that their frame is not the last one, which means the picture is several frames composed in order with the blending each states — and this reader was reading the first and stopping. Handing that back would be a picture nobody encoded, so a file with more than one frame is now refused by name rather than by accident. That is the honest state of it: the splines in those files decode, and what is missing is the composition around them.
...
- The JPEG XL pixel path is measured against libjxl on 104 files, not against every `.jxl` that exists; multi-frame files, patches and noise are refused rather than decoded. Do not treat its internal round-trip as proof of arbitrary `.jxl` compatibility.
