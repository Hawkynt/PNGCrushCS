...
| JPEG XL | ⚠️ | — | Container, `SizeHeader`, `ImageMetadata` and `FrameHeader` are spec-conformant, so signature, dimensions and image-level metadata of real `.jxl` files are correct. Both pixel codecs decode real `cjxl` output. Modular does it sample for sample across the whole effort range, at 8-bit RGB and grayscale, in one group or several. VarDCT — the lossy one — agrees with libjxl to within **0.011 of one eight-bit level** across 49 files from 16x16 to 1,024x768; comparing after eight-bit rounding, 290 samples of 6,174,252 differ and none by more than one level. Lossy frames used to be refused on a comparison against `djxl`'s `.ppm` output, which is dithered on purpose and so was measuring libjxl's blue noise rather than this decoder; they are returned now. 16-bit files are carried at their own depth as `Rgb48`/`Gray16` rather than narrowed, and **all seven in the corpus decode byte for byte against `djxl`** — libjxl dithers only 8-bit output, so at that depth there is nothing between the two decoders at all. Splines — the thin curves a frame draws on top of its blocks — are decoded and rendered, and a 2,048-square file of 81 bytes that is nothing else agrees with libjxl's float decode in all but 52 of its 12,582,912 samples, every one of those on a rounding boundary. Files of more than one frame are composed rather than read to the first: a still picture is every frame drawn in the order and blending each states, and an animation is its first shown frame. The noise layer is decoded and generated: a frame that asks for film grain gets libjxl's own generator, seeded the way libjxl seeds it, and a photon-noise file matches its float decode in every one of its 9,216 samples. Patches are decoded and stamped, including the kept-aside frames they are cut from; a file of one repeated motif matches libjxl's float decode in all but 3 of 98,304 samples, all three on a rounding boundary. A picture carrying an embedded colour profile opens too — the profile follows the transform bundle rather than the metadata, and reading it a bundle early meant such a file could not be opened at all. **Every fixture and every one of the 104 corpus files now decodes, and nothing in the format is refused any more.** The writer is unregistered because `djxl` rejects what it emits. |
