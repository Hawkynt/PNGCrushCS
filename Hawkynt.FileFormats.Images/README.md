...
- The JPEG XL pixel path is measured against libjxl on 49 lossy and 55 lossless files, not against every `.jxl` that exists; 16-bit samples and a few frame features, splines among them, are refused rather than decoded. Do not treat its internal round-trip as proof of arbitrary `.jxl` compatibility.
