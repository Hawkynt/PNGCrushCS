...
What that filter is, as far as it has been read: three passes over the picture, the first weighing twelve neighbours and the second and third four each, every neighbour's weight falling off with a sum of absolute differences taken over a five-point cross rather than at the single pixel, and the three passes averaging into the same running total. Its strength is per block, from the sharpness plane the metadata already carries here and the block's own quantisation step. The numbers it takes are a channel weighting of forty, five and three and a half, thresholds of 0.45 and 0.6 below which a pass contributes nothing, a quantisation multiplier of 0.46, and eight sharpness levels spread evenly from nothing to one. What is not settled is the exact expression that turns a block's quantisation step into its strength, and whether the filter runs before or after the smoothing one. Both of those decide whether the filter helps or hurts, and the last attempt at connecting one that was close rather than right measured net-neutral across seven files and made one of them worse — so it stays unconnected until it can be written from the definition rather than from the shape of it.
