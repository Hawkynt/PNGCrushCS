...
/// <summary>Reads ZX Spectrum screen files from bytes, streams, or file paths.</summary>
public static class ZxSpectrumReader {

  /// <summary>Total file size: 6144 bitmap + 768 attribute bytes.</summary>
  ...
  public static ZxSpectrumFile FromFile(FileInfo file) {
    ArgumentNullException.ThrowIfNull(file);
    if (!file.Exists)
      throw new FileNotFoundException("ZX Spectrum screen file not found.", file.FullName);

    return FromBytes(File.ReadAllBytes(file.FullName));
  }

  public static ZxSpectrumFile FromStream(Stream stream) => FromBytes(StreamBytes.ReadAll(stream));

  public static ZxSpectrumFile FromSpan(ReadOnlySpan<byte> data) {

    if (data.Length != FileSize)
      throw new InvalidDataException($"ZX Spectrum screen must be exactly {FileSize} bytes, got {data.Length}.");
