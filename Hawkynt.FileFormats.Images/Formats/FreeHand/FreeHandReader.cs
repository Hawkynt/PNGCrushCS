...
/// <summary>Reads FreeHand ST files from bytes, streams, or file paths.</summary>
public static class FreeHandReader {
  ...
  public static FreeHandFile FromFile(FileInfo file) {
    ArgumentNullException.ThrowIfNull(file);
    if (!file.Exists)
      throw new FileNotFoundException("FreeHand ST file not found.", file.FullName);

    return FromBytes(File.ReadAllBytes(file.FullName));
  }

  public static FreeHandFile FromStream(Stream stream) => FromBytes(StreamBytes.ReadAll(stream));
