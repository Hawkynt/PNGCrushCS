...
public static class MobyDickReader {
  ...
  public static MobyDickFile FromStream(Stream stream) => FromBytes(StreamBytes.ReadAll(stream));

  public static MobyDickFile FromSpan(ReadOnlySpan<byte> data) {
    ...
    data.Slice(offset, MobyDickFile.PaletteDataSize).CopyTo(palette.AsSpan(0));
    offset += MobyDickFile.PaletteDataSize;

    // Pixel data (64000 bytes)
    var pixelData = new byte[MobyDickFile.PixelDataSize];
    data.Slice(offset, MobyDickFile.PixelDataSize).CopyTo(pixelData.AsSpan(0));
