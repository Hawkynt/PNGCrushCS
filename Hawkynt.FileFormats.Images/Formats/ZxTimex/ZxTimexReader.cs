using System;
using System.IO;

namespace FileFormat.ZxTimex;

/// <summary>Reads Timex HiColor (.tmx) files from bytes, streams, or file paths.</summary>
public static class ZxTimexReader {

  /// <summary>Total file size: 6144 bitmap + 6144 extended attributes.</summary>
  internal const int FileSize = 12288;

  /// <summary>Bitmap data size in bytes.</summary>
  internal const int BitmapSize = 6144;

  /// <summary>Extended attribute data size in bytes (32 * 192).</summary>
  internal const int AttributeSize = 6144;

  /// <summary>Bytes per pixel row.</summary>
  internal const int BytesPerRow = 32;

  /// <summary>Number of pixel rows.</summary>
  internal const int RowCount = 192;

  public static ZxTimexFile FromFile(FileInfo file) {
    ArgumentNullException.ThrowIfNull(file);
    if (!file.Exists)
      throw new FileNotFoundException("Timex HiColor file not found.", file.FullName);

    return FromBytes(File.ReadAllBytes(file.FullName));
  }

  public static ZxTimexFile FromStream(Stream stream) => FromBytes(StreamBytes.ReadAll(stream));

  public static ZxTimexFile FromSpan(ReadOnlySpan<byte> data) {

    if (data.Length != FileSize)
      throw new InvalidDataException($"Timex HiColor file must be exactly {FileSize} bytes, got {data.Length}.");

    var linearBitmap = new byte[BitmapSize];

    // Deinterleave from ZX Spectrum memory layout to linear row order
    for (var y = 0; y < RowCount; ++y) {
      var third = y / 64;
      var characterRow = (y % 64) / 8;
      var pixelLine = y % 8;
      var srcOffset = third * 2048 + pixelLine * 256 + characterRow * BytesPerRow;
      var dstOffset = y * BytesPerRow;
      data.Slice(srcOffset, BytesPerRow).CopyTo(linearBitmap.AsSpan(dstOffset));
    }

    var attributes = new byte[AttributeSize];
    data.Slice(BitmapSize, AttributeSize).CopyTo(attributes.AsSpan(0));

    return new ZxTimexFile {
      BitmapData = linearBitmap,
      AttributeData = attributes,
    };
    }

  public static ZxTimexFile FromBytes(byte[] data) {
    ArgumentNullException.ThrowIfNull(data);
    return FromSpan(data);
  }
}
