...
public static class TimReader {
  ...
  public static TimFile FromStream(Stream stream) => FromBytes(StreamBytes.ReadAll(stream));

  public static TimFile FromSpan(ReadOnlySpan<byte> data) {
    ...
    if (hasClut) {
      ...
      data.Slice(offset, clutDataSize).CopyTo(clutData.AsSpan(0));
      offset += clutDataSize;
    }
