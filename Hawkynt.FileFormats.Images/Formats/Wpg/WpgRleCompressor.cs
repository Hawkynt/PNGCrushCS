...
internal static class WpgRleCompressor {
  ...
  /// <summary>Compresses a bitmap a scanline at a time, which is how WPG stores one.</summary>
  /// <remarks>
  /// Each row is its own little stream: no run carries over into the next, and a reader that stops at
  /// the end of a row still lands on a control byte. Compressing the whole raster in one pass would
  /// merge the runs across that boundary and put every row after the first out of step.
  /// </remarks>
