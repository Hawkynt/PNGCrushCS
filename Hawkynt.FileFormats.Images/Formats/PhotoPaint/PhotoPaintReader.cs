...
public static class PhotoPaintReader {
  ...
  public static PhotoPaintFile FromStream(Stream stream) => FromBytes(StreamBytes.ReadAll(stream));

  public static PhotoPaintFile FromSpan(ReadOnlySpan<byte> data) {
    ...
    data.Slice(PhotoPaintFile.HeaderSize, available).CopyTo(pixelData.AsSpan(0));
