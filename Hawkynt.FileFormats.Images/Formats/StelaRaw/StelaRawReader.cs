...
public static class StelaRawReader {
  ...
  public static StelaRawFile FromStream(Stream stream) => FromBytes(StreamBytes.ReadAll(stream));
  ...
  public static StelaRawFile FromSpan(ReadOnlySpan<byte> data) {
    ...
    var available = data.Length - StelaRawFile.HeaderSize;
    if (available < pixelBytes)
      throw new InvalidDataException($"Expected {pixelBytes} bytes of pixel data, got {available}.");

    data.Slice(StelaRawFile.HeaderSize, pixelBytes).CopyTo(pixelData.AsSpan(0));
