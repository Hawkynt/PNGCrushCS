...
public static class ZxChrdReader {
  ...
  public static ZxChrdFile FromStream(Stream stream) => FromBytes(StreamBytes.ReadAll(stream));

  public static ZxChrdFile FromSpan(ReadOnlySpan<byte> data) {
    ...
    data.Slice(0, FileSize).CopyTo(charData);
