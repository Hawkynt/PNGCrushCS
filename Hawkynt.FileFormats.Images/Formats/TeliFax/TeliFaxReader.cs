...
public static class TeliFaxReader {
  ...
  public static TeliFaxFile FromStream(Stream stream) => FromBytes(StreamBytes.ReadAll(stream));

  public static TeliFaxFile FromSpan(ReadOnlySpan<byte> data) {
    ...
    data.Slice(TeliFaxFile.HeaderSize, pixelDataSize).CopyTo(pixelData.AsSpan(0));
