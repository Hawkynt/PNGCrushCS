...
/// <summary>Reads ZX Spectrum ULAplus (.ulp) files from bytes, streams, or file paths.</summary>
public static class ZxUlaPlusReader {

  /// <summary>Total file size: 6144 bitmap + 768 attributes + 64 palette entries.</summary>
  internal const int FileSize = 6976;
  ...
  /// <summary>ULAplus palette data size in bytes.</summary>
  internal const int PaletteSize = 64;
  ...
  public static ZxUlaPlusFile FromFile(FileInfo file) {
    ArgumentNullException.ThrowIfNull(file);
    if (!file.Exists)
      throw new FileNotFoundException("ZX Spectrum ULAplus file not found.", file.FullName);

    return FromBytes(File.ReadAllBytes(file.FullName));
  }

  public static ZxUlaPlusFile FromStream(Stream stream) => FromBytes(StreamBytes.ReadAll(stream));

  public static ZxUlaPlusFile FromSpan(ReadOnlySpan<byte> data) {
    ...
    data.Slice(BitmapSize + AttributeSize, PaletteSize).CopyTo(palette.AsSpan(0));
