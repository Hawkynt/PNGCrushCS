...
public static class SpritePadReader {
  ...
  public static SpritePadFile FromStream(Stream stream) => FromBytes(StreamBytes.ReadAll(stream));

  public static SpritePadFile FromSpan(ReadOnlySpan<byte> data) {
    ...
    if (version >= 2 && data.Length >= SpritePadFile.V2HeaderSize + SpritePadFile.BytesPerSprite) {
      ...
      data.Slice(SpritePadFile.V1HeaderSize, extraHeader.Length).CopyTo(extraHeader.AsSpan());
    } else {
    ...
    data.Slice(headerSize, payloadSize).CopyTo(rawData.AsSpan());
