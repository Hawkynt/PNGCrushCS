...
public static class GigaPaintReader {
  ...
  public static GigaPaintFile FromStream(Stream stream) => FromBytes(StreamBytes.ReadAll(stream));

  public static GigaPaintFile FromSpan(ReadOnlySpan<byte> data) {
    ...
    data.Slice(GigaPaintFile.LoadAddressSize, rawData.Length).CopyTo(rawData.AsSpan(0));
