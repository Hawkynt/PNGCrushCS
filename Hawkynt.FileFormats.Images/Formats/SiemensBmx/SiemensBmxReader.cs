...
public static class SiemensBmxReader {
  ...
  public static SiemensBmxFile FromStream(Stream stream) => FromBytes(StreamBytes.ReadAll(stream));
  ...
  public static SiemensBmxFile FromSpan(ReadOnlySpan<byte> data) {
    ...
    var available = data.Length - SiemensBmxFile.HeaderSize;
    if (available < pixelBytes)
      throw new InvalidDataException($"Expected {pixelBytes} bytes of pixel data, got {available}.");

    data.Slice(SiemensBmxFile.HeaderSize, pixelBytes).CopyTo(pixelData.AsSpan(0));
