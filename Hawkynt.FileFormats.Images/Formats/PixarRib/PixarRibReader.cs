...
/// <summary>Reads Pixar RIB texture files from bytes, streams, or file paths.</summary>
public static class PixarRibReader {

  public static PixarRibFile FromFile(FileInfo file) {
    ArgumentNullException.ThrowIfNull(file);
    if (!file.Exists)
      throw new FileNotFoundException("PixarRib file not found.", file.FullName);
    return FromBytes(File.ReadAllBytes(file.FullName));
  }

  public static PixarRibFile FromStream(Stream stream) => FromBytes(StreamBytes.ReadAll(stream));

  public static PixarRibFile FromSpan(ReadOnlySpan<byte> data) {

    if (data.Length < PixarRibFile.HeaderSize)
      throw new InvalidDataException("Data too small for a valid PixarRib file.");
    ...
    var available = data.Length - PixarRibFile.HeaderSize;
    if (available < pixelBytes)
      throw new InvalidDataException($"Expected {pixelBytes} bytes of pixel data, got {available}.");

    data.Slice(PixarRibFile.HeaderSize, pixelBytes).CopyTo(pixelData.AsSpan(0));
