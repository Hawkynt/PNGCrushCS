...
public static class WbmpReader {
  ...
  public static WbmpFile FromStream(Stream stream) => FromBytes(StreamBytes.ReadAll(stream));

  public static WbmpFile FromSpan(ReadOnlySpan<byte> data) {
    ...
    data.Slice(offset, expectedPixelBytes).CopyTo(pixelData.AsSpan(0));
