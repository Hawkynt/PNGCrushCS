...
public static class PcxReader {
  ...
  public static PcxFile FromStream(Stream stream) => FromBytes(StreamBytes.ReadAll(stream));

  public static PcxFile FromSpan(ReadOnlySpan<byte> data) {
    ...
    if (encoding == 1) {
      ...
      data.Slice(offset, remaining).CopyTo(decodedScanlines.AsSpan(0));
    }
    ...
    if (numPlanes == 3 && bitsPerPixel == 8) {
      ...
      if (data.Length >= 769) {
        ...
        if (data[paletteMarkerPos] == 0x0C) {
          ...
          data.Slice(paletteMarkerPos + 1, 768).CopyTo(palette.AsSpan(0));
