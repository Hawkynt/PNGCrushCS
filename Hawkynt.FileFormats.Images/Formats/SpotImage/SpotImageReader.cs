...
public static class SpotImageReader {
  ...
  public static SpotImageFile FromStream(Stream stream) => FromBytes(StreamBytes.ReadAll(stream));

  public static SpotImageFile FromSpan(ReadOnlySpan<byte> data) {
    ...
    if (available > 0)
      data.Slice(SpotImageFile.HeaderSize, available).CopyTo(pixelData.AsSpan(0));
