...
/// <summary>Reads OazFax OAZ/XFX files from bytes, streams, or file paths.</summary>
public static class OazFaxReader {

  public static OazFaxFile FromFile(FileInfo file) {
    ArgumentNullException.ThrowIfNull(file);
    if (!file.Exists)
      throw new FileNotFoundException("OAZ file not found.", file.FullName);

    return FromBytes(File.ReadAllBytes(file.FullName));
  }

  public static OazFaxFile FromStream(Stream stream) => FromBytes(StreamBytes.ReadAll(stream));
  ...
  public static OazFaxFile FromSpan(ReadOnlySpan<byte> data) {
    ...
    if (data.Length < OazFaxFile.HeaderSize + pixelDataSize)
