...
/// <summary>Reads ZX Spectrum Gigascreen (.gsc) files from bytes, streams, or file paths.</summary>
public static class ZxGigascreenReader {

  /// <summary>Total file size: two complete 6912-byte screens.</summary>
  internal const int FileSize = 13824;
  ...
  public static ZxGigascreenFile FromFile(FileInfo file) {
    ArgumentNullException.ThrowIfNull(file);
    if (!file.Exists)
      throw new FileNotFoundException("ZX Spectrum Gigascreen file not found.", file.FullName);

    return FromBytes(File.ReadAllBytes(file.FullName));
  }

  public static ZxGigascreenFile FromStream(Stream stream) => FromBytes(StreamBytes.ReadAll(stream));

  public static ZxGigascreenFile FromSpan(ReadOnlySpan<byte> data) {
    if (data.Length != FileSize)
      throw new InvalidDataException($"ZX Spectrum Gigascreen file must be exactly {FileSize} bytes, got {data.Length}.");
