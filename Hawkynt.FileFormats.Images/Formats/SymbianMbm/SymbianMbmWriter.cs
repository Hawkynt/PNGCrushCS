...
public static class SymbianMbmWriter {
  ...
  internal static byte[] Assemble(SymbianMbmFile file) {
    ...
    for (var i = 0; i < bitmapCount; ++i) {
      ...
      // iBitmapSize is the whole bitmap, this header included - what CBitwiseBitmap::Construct puts
      // there. Twips stay 0, as they do on a bitmap nothing gave a physical size to.
      var bmpHeader = new SymbianMbmBitmapHeader(
        bitmapTotalSize,
        SymbianMbmFile.BitmapHeaderSize,
        bmp.Width, bmp.Height,
        bmp.WidthInTwips, bmp.HeightInTwips,
        bmp.BitsPerPixel,
        bmp.ColorMode, bmp.PaletteSize, bmp.Compression
      );
