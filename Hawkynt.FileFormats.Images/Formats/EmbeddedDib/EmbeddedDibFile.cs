...
/// <remarks>
/// A whole family of formats are not pictures at all — Corel's metafile, Zoner's metafile, AutoDesk
/// sketch thumbnails, IntroCAD drawings, jigsaw and button projects — and every one of them carries
/// a preview so a file chooser has something to show. That preview is a plain Windows DIB dropped
/// into the file: a 40-byte <c>BITMAPINFOHEADER</c>, its palette, and its rows bottom-up and padded
/// to four bytes, with no <c>BM</c> file header in front of it because it is not a file.
/// <para/>
/// Seven of the names on the coverage list turned out to be this one thing, which is why it is a
/// reader of its own rather than seven hard-coded offsets. It finds the header by looking for one —
...
/// <para/>
/// An eighth, <c>.jig</c>, is left out deliberately. It carries a header of the right shape at 14 and
...
/// writers such as <see cref="FileFormat.Cdr.CdrWriter"/> can use <see cref="EmbeddedDibWriter"/> to
/// serialize the packed DIB payload they embed.
/// </remarks>
