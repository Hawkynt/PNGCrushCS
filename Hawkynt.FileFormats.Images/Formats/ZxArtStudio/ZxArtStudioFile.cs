...
public readonly record struct ZxArtStudioFile
  : IImageFormatReader<ZxArtStudioFile>, IImageToRawImage<ZxArtStudioFile>,
    IImageFromRawImage<ZxArtStudioFile>, IImageFormatWriter<ZxArtStudioFile> {
  ...
  static VideoMode[] IImageFormatMetadata<ZxArtStudioFile>.VideoModes => [
  ...
  /// <summary>Builds an Art Studio screen from a <see cref="RawImage"/>. Every pixel is mapped onto the
  ...
  public static ZxArtStudioFile FromRawImage(RawImage image) {
    ...
    if (image.Width != 256 || image.Height != 192)
      throw new ArgumentException($"ZX Spectrum Art Studio screens are always 256x192, but got {image.Width}x{image.Height}.", nameof(image));
