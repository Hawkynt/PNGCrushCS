...
public static class SeqImageReader {
  ...
  public static SeqImageFile FromStream(Stream stream) => FromBytes(StreamBytes.ReadAll(stream));

  public static SeqImageFile FromSpan(ReadOnlySpan<byte> data) {
    ...
    if (pixelDataSize > 0)
      data.Slice(SeqImageFile.HeaderSize, pixelDataSize).CopyTo(pixelData.AsSpan(0));
