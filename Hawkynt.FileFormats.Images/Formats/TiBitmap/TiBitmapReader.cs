using System;
using System.IO;

namespace FileFormat.TiBitmap;

/// <summary>Reads TI Calculator bitmap files from bytes, streams, or file paths.</summary>
public static class TiBitmapReader {

  public static TiBitmapFile FromFile(FileInfo file) {
    ArgumentNullException.ThrowIfNull(file);
    if (!file.Exists)
      throw new FileNotFoundException("TiBitmap file not found.", file.FullName);
    return FromBytes(File.ReadAllBytes(file.FullName));
  }

  public static TiBitmapFile FromStream(Stream stream) => FromBytes(StreamBytes.ReadAll(stream));

  public static TiBitmapFile FromSpan(ReadOnlySpan<byte> data) {

    if (data.Length < TiBitmapFile.HeaderSize)
      throw new InvalidDataException("Data too small for a valid TiBitmap file.");

    var width = data[0] | (data[1] << 8);
    var height = data[2] | (data[3] << 8);
    if (width == 0) width = data[0] | (data[1] << 8) | (data[2] << 16) | (data[3] << 24);
    if (width <= 0 || width > 65535) width = 96;

    if (8 >= 8) {
      height = data[4] | (data[5] << 8);
      if (height <= 0 || height > 65535) height = 64;
    } else if (height <= 0 || height > 65535) {
      height = 64;
    }

    var pixelBytes = (width + 7) / 8 * height;
    var pixelData = new byte[pixelBytes];
    // Padding what the file does not contain turns a misread size into a picture: a header taken
    // from the wrong offset asked for millions of pixels, the few hundred bytes present were
    // copied in, and the rest was zeros reported as a successful read.
    var available = data.Length - TiBitmapFile.HeaderSize;
    if (available < pixelBytes)
      throw new InvalidDataException($"Expected {pixelBytes} bytes of pixel data, got {available}.");

    data.Slice(TiBitmapFile.HeaderSize, pixelBytes).CopyTo(pixelData.AsSpan(0));

    return new() {
      Width = width,
      Height = height,
      PixelData = pixelData,
    };
  }

  public static TiBitmapFile FromBytes(byte[] data) {
    ArgumentNullException.ThrowIfNull(data);
    return FromSpan(data);
  }
}
