...
public static class RedStormRsbReader {
  ...
  public static RedStormRsbFile FromStream(Stream stream) => FromBytes(StreamBytes.ReadAll(stream));

  public static RedStormRsbFile FromSpan(ReadOnlySpan<byte> data) {
    ...
    data.Slice(RedStormRsbFile.HeaderSize, pixelDataSize).CopyTo(pixelData.AsSpan(0));
