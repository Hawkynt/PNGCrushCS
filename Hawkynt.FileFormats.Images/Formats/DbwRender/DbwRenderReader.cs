...
public static class DbwRenderReader {
  ...
  public static DbwRenderFile FromStream(Stream stream) => FromBytes(StreamBytes.ReadAll(stream));

  public static DbwRenderFile FromSpan(ReadOnlySpan<byte> data) {
    ...
    data.Slice(DbwRenderFile.HeaderSize, expectedPixelBytes).CopyTo(pixelData.AsSpan(0));
