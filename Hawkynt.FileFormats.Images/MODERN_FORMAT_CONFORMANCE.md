...
## Known VP8L reader gap

`Vp8LPredictorTransform` does not reproduce libwebp's predictors. Predictors 5 to 9 are mapped to the wrong
neighbour pairs (5 must average left, top and top-right; 6 is left with top-left; 7 is left with top; 8 is
top-left with top; 9 is top with top-right), predictor 11's tie-break returns the left neighbour where libwebp
returns the top one, and the top-right neighbour of the last column must be the current row's first pixel
rather than the pixel above. Streams written by this repository are unaffected — its VP8L encoder emits only
the subtract-green transform — but a `cwebp -lossless` picture that uses a predictor transform decodes with
small per-sample errors, and so does the `ALPH` chunk of a `cwebp` lossy-plus-alpha file: 172 of 2048 alpha
samples differ on the measured 32x64 case. Reading such files exactly requires correcting the predictor table
first.
